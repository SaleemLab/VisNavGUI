...
if FcircularY
    if strcmp(varYname,'LFPphase') || strcmp(varYname,'LFPphase2') || strcmp(varYname,'spikePhase')
        minvarYname = 0;
        maxvarYname = 360;
    else
    ...
    for k = 1:size(obj.data.es.(varYname),2)
        ...
        Ytemp(~isnan(Ytemp)) = smthInTime(Ytemp(~isnan(Ytemp)), sampleRate, Tsmthwin, 'same', [], 'boxcar_centered');
        varY(:,k) = mod(Ytemp,maxvarYname);
    end
else
    for k = 1:size(obj.data.es.(varYname),2)
        Ytemp = obj.data.es.(varYname)(:,k);
        Ytemp(~isnan(Ytemp)) = smthInTime(Ytemp(~isnan(Ytemp)), sampleRate, Tsmthwin, 'same', [], 'boxcar_centered');
        varY(:,k) = Ytemp;%smthInTime(obj.data.es.(varYname), sampleRate, Tsmthwin, 'same', [], 'boxcar_centered');
    end
end

numBinsX = round(maxvarXname/Xbinsize);%floor(max(varX(:))/Xbinsize) + 1;
for k = 1:size(obj.data.es.(varXname),2)
...
%     end
%     varY = varYBinned;
elseif strcmp(varYname,'gain')
    [gainval,~,ic] = unique(varY(~isnan(varY)),'sorted');
    varY(~isnan(varY)) = ic;
    minvarYname = min(varY(~isnan(varY)));
    maxvarYname = max(varY(~isnan(varY)));
    numBinsY = numel(unique(varY(~isnan(varY))));%floor(max(varY(:))/Ybinsize) + 1;
else        
    numBinsY = round(maxvarYname/Ybinsize);%floor(max(varY(:))/Ybinsize) + 1;
end
for k = 1:size(obj.data.es.(varYname),2)
...
gbase = find(obj.SubsetVal.gain == mode(obj.data.es.gain));
cbase = nbcont;%all contrast pooled together
gainlist = 1:nbgain;
    ...
    for g = gainlist
        for r = 1:nbroomlength
            for o = 1:nboutcome
                ...
                obj.maps2d.([varXname '_' varYname]){c, g, r, o}.sampleRate = obj.data.es.sampleRate;
                ...
                obj.maps2d.([varXname '_' varYname]){c, g, r, o}.kfold = 1;
                obj.maps2d.([varXname '_' varYname]){c, g, r, o}.Fdiscarditer = true;
                obj.maps2d.([varXname '_' varYname]){c, g, r, o}.Fgoodcells = obj.CellInfo.Goodcluster;
                if strcmp(varYname,'gain')
                    obj.maps2d.([varXname '_' varYname]){c, g, r, o}.refYindex = gbase;
                else
                    obj.maps2d.([varXname '_' varYname]){c, g, r, o}.refYindex = [];
                end
                ...
                if ~strcmp(varYname,'gain')
                    ...
                    idx = false(size(obj.data.es.(varYname)));
                    for gg = gainlist
                ...
                if o == 3 && c == nbcont
                    if sum(idx) > size_th
                        if ~strcmp(varYname,'gain')
                            if g ~= gbase
                                obj.maps2d.([varXname '_' varYname]){c, g, r, o}.tuningRef = obj.maps2d.([varXname '_' varYname]){cbase, gbase, r, o}.model.tuning;
                            else
                                obj.maps2d.([varXname '_' varYname]){c, g, r, o}.tuningRef = [];
                            end
                ...
                obj.maps2d.([varXname '_' varYname '_Shf']){c, g, r, o} = TtwoDimMap('Xsmth_win', floor(Xsmthwin/Xbinsize), 'Ysmth_win', floor(Ysmthwin/Ybinsize));
                obj.maps2d.([varXname '_' varYname '_Shf']){c, g, r, o}.sampleRate = obj.data.es.sampleRate;
                obj.maps2d.([varXname '_' varYname '_Shf']){c, g, r, o}.FcircularX = FcircularX;
                obj.maps2d.([varXname '_' varYname '_Shf']){c, g, r, o}.FcircularY = FcircularY;
                obj.maps2d.([varXname '_' varYname '_Shf']){c, g, r, o}.numBinsX = numBinsX;
                obj.maps2d.([varXname '_' varYname '_Shf']){c, g, r, o}.numBinsY = numBinsY;
                obj.maps2d.([varXname '_' varYname '_Shf']){c, g, r, o}.binsX = binsX;
                obj.maps2d.([varXname '_' varYname '_Shf']){c, g, r, o}.binsY = binsY;
                obj.maps2d.([varXname '_' varYname '_Shf']){c, g, r, o}.qthreshold = 1;
                obj.maps2d.([varXname '_' varYname '_Shf']){c, g, r, o}.Fdiscarditer = true;
                obj.maps2d.([varXname '_' varYname '_Shf']){c, g, r, o}.FshuffleXY = true;
                obj.maps2d.([varXname '_' varYname '_Shf']){c, g, r, o}.Fgoodcells = obj.CellInfo.Goodcluster;
                obj.maps2d.([varXname '_' varYname '_Shf']){c, g, r, o}.kfold = nShf;
                if strcmp(varYname,'gain')
                    obj.maps2d.([varXname '_' varYname '_Shf']){c, g, r, o}.refYindex = gbase;
                else
                    obj.maps2d.([varXname '_' varYname '_Shf']){c, g, r, o}.refYindex = [];
                end
                ...
                if o == 3 && c == nbcont
                    if sum(idx) > size_th
                        if ~strcmp(varYname,'gain')
                            if g ~= gbase
                                obj.maps2d.([varXname '_' varYname '_Shf']){c, g, r, o}.tuningRef = obj.maps2d.([varXname '_' varYname]){cbase, gbase, r, o}.model.tuning;
                            else
                                obj.maps2d.([varXname '_' varYname '_Shf']){c, g, r, o}.tuningRef = [];
                            end
                            obj.maps2d.([varXname '_' varYname '_Shf']){c, g, r, o}.trainSpikeMap(varX(idx,:), varY(idx,:), spiketrain(idx,:), T(idx));
                        else
                            if g == gbase
                                obj.maps2d.([varXname '_' varYname '_Shf']){c, g, r, o}.tuningRef = obj.maps2d.([varXname '_' varYname]){cbase, gbase, r, o}.model.tuning;
                                obj.maps2d.([varXname '_' varYname '_Shf']){c, g, r, o}.trainSpikeMap(varX(idx,:), varY(idx,:), spiketrain(idx,:), T(idx));
                ...
                if ~strcmp(varYname,'gain')
                    ...
                    obj.maps2d.([varXname '_' varYname '_2fold']){c, g, r, o}.sampleRate = obj.data.es.sampleRate;
                    ...
                    obj.maps2d.([varXname '_' varYname '_2fold']){c, g, r, o}.Fgoodcells = obj.CellInfo.Goodcluster;
