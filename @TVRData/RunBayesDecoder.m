...
if nargin < 4
    ...
    prompt = {'Use lookup table';'Window size (ms)';'Spatial smth (%)';'# of X bins';'How many theta phase bins';'theta channel#';'How many speed bins';'How many eye bins';'Field smooth win. (ms)';'Latency shift (ms)';'alpha';'delta (ms)';'How many runs';'Error threshold';'# folds';'Optimal smoothing?';'Shuffle theta bins?';'Shuffle Speed bins?'};
    dlg_title = 'Decoder Parameters';
    num_lines = 1;
    def = {'0';'250';'4';'100';'1';'34';'3';'3';'15';'0';'0';'0';'1';'30';'20';'0';'0';'0'};
    ...
    if ~isempty(nruns)
        ...
        obj.Bayes.FshuffleThetaPhs = boolean(str2num(nruns{17}));
        obj.Bayes.FshuffleSpdbins = boolean(str2num(nruns{18}));
    else
        ...
        obj.Bayes.kfold = 20;
        obj.Bayes.FoptiSmooth = false;
        obj.Bayes.FshuffleThetaPhs = false;
        obj.Bayes.FshuffleSpdbins = false;
    end
...
% obj.Bayes.Posterior1 = cell(nbProbe,1);
% obj.Bayes.PosError1 = cell(nbProbe,1);
obj.Bayes.X0 = cell(1);

nShf = size(obj.CellInfo.fieldShfAmpiter{cbase, gbase, rbase, obase},2);
signiAmpfields = sum(abs(repmat(obj.CellInfo.fieldAmp{cbase, gbase, rbase, obase}(:),[1 nShf])) < obj.CellInfo.fieldShfAmpiter{cbase, gbase, rbase, obase},2)/nShf;
signiAmpfields = (signiAmpfields < 0.01)';
signiAmpfields = true(size(signiAmpfields));

for iprobe = 1:nbProbe
    if iprobe == 1 %CA1
        obj.Bayes.DecCellidx{iprobe} = signiSinfoperSpike & stableplacefields & signiplacefields & SSIrangeidx & goodunits & CA1cells & ~CA1interneurons & signiAmpfields;
    elseif iprobe == 2 %V1
        obj.Bayes.DecCellidx{iprobe} =   signiSinfoperSpike & stableplacefields & signiplacefields & SSIrangeidx & goodunits & ~CA1cells & signiAmpfields;
    end
end
...
%         X = getBayesVar(obj.data.es.(obj.Bayes.varname),obj.Bayes.latcorrection(iprobe),sampleRate,obj.Bayes.Tsmth_win,smthtype,obj.data.es.CircularMaze,maxX);
        ...
        Xcell = repmat(X(:),[1 sum(obj.Bayes.DecCellidx{iprobe})]);
%         if strcmp(obj.Bayes.varname,'trajPercent')
%             Xcell = getBayesVar(obj.data.es.spiketrajPercent(:,obj.Bayes.DecCellidx{iprobe}),obj.Bayes.latcorrection(iprobe),sampleRate,obj.Bayes.Tsmth_field,smthtype,obj.data.es.CircularMaze,maxX);
%             [Xcell, ~] = normalise1var(Xcell, obj.Bayes.numBins,[],[0 maxX]);
%         else
%             Xcell = repmat(X(:),[1 sum(obj.Bayes.DecCellidx{iprobe})]);
%         end
        
        Xfilt = getBayesVar(obj.data.es.(obj.Bayes.varname),obj.Bayes.latcorrection(iprobe),sampleRate,obj.Bayes.Tsmth_win,smthtype,obj.data.es.CircularMaze,maxX);
        [Xfilt, ~] = normalise1var(Xfilt, obj.Bayes.numBins,[],[0 maxX]);
        obj.Bayes.Xsmth = Xfilt(:);
        
        Y0 = obj.data.es.(obj.Bayes.predictorname);
        
        win = round(obj.Bayes.Tsmth_win/(1000/sampleRate));
        T0 = obj.data.es.sampleSize;%.*win;
...
%             obj.Bayes.LFPphase{iprobe} = getBayesVar(obj.data.es.LFPphase2(:,iprobe),obj.Bayes.latcorrection(iprobe),sampleRate,obj.Bayes.Tsmth_field,smthtype,true,360);
        ...
        itraj = obj.Bayes.X;%floor(X/(max(round(X))/obj.Bayes.numBins))+1;
        ntrajbins = max(itraj);
...
%         end
        ...
        Y = cell(obj.Bayes.nthetaphsbins,1);
        T = cell(obj.Bayes.nthetaphsbins,1);
        for iphs = 1:obj.Bayes.nthetaphsbins
            Y{iphs} = Ytemp;
            T{iphs} = T0;
%             phsidx = obj.Bayes.Phsbin{1} == iphs;
%             Y{iphs}(~phsidx,:) = 0;
%             T{iphs}(~phsidx) = 0;
        end
...
%         end
        ...
        nbCells = size(Y{iphs},2);
        Yfilt = cell(obj.Bayes.nthetaphsbins,1);
        Tfilt = cell(obj.Bayes.nthetaphsbins,1);
        for iphs = 1:obj.Bayes.nthetaphsbins
            Yfilt{iphs} = zeros(size(Y{iphs}));
            for icell = 1:nbCells
                Yfilt{iphs}(:,icell) = smthInTime(Y{iphs}(:,icell), sampleRate, obj.Bayes.Tsmth_win, 'same', [], wintype);%, 'boxcarsum');
            end
            Tfilt{iphs} = smthInTime(T{iphs}, sampleRate, obj.Bayes.Tsmth_win, 'same', [], wintype);%, 'boxcarsum');
        end

        winsmth_spd = round(obj.Bayes.Tsmth_field/(1000/sampleRate));
        for iphs = 1:obj.Bayes.nthetaphsbins
            for icell = 1:nbCells
                Y{iphs}(:,icell) = smthInTime(Y{iphs}(:,icell), sampleRate, obj.Bayes.Tsmth_field, 'same', [], wintype);%, 'boxcarsum');
                Y{iphs}(:,icell) = Y{iphs}(:,icell);
            end
            T{iphs} = smthInTime(T{iphs}, sampleRate, obj.Bayes.Tsmth_field, 'same', [], wintype);%, 'boxcarsum');
        end
...
%         obj.Bayes.Posterior1{iprobe,1} = zeros(numel(X),obj.Bayes.numBins);
%         obj.Bayes.PosError1{iprobe,1} = zeros(numel(X),obj.Bayes.numBins);
%         obj.Bayes.MaxDecodedPosition1{iprobe,1} = zeros(numel(X),1);
%         obj.Bayes.MeanDecodedPosition1{iprobe,1} = zeros(numel(X),1);
%         obj.Bayes.DecodingError1{iprobe,1} = zeros(numel(X),1);
%         obj.Bayes.DecodingBias1{iprobe,1} = zeros(numel(X),1);
        
        
        Ycell = Y0(:,obj.Bayes.DecCellidx{iprobe});
        if obj.Bayes.FshuffleThetaPhs
            disp('Randomized theta phase bins');
            nspdbins = max(obj.Bayes.Spdbin);
            neyebins = max(obj.Bayes.Eyebin);
            for spd = 1:nspdbins
                for eye = 1:neyebins
                    spdidx = (obj.Bayes.Spdbin == spd);
                    eyeidx = (obj.Bayes.Eyebin == eye);
                    spdidx = spdidx & eyeidx;
                    spdidx = spdidx(:);
                    alltestidx = cell(obj.Bayes.kfold,1);
                    for iter = 1:obj.Bayes.kfold
                        alltestidx{iter} = false(size(idxref));
                    end
                    for phs = 1:obj.Bayes.nthetaphsbins
                        phsidx = obj.Bayes.Phsbin{1} == phs;
                        idxtemp = find(idxref & spdidx & phsidx);
                        CVO = crossValPartition(ones(1,numel(idxtemp)),obj.Bayes.kfold);
                        for iter = 1:obj.Bayes.kfold
                            alltestidx{iter}(idxtemp(CVO.test{iter})) = true;
                        end
                    end
                    for iter = 1:obj.Bayes.kfold
                        phsbintemp = obj.Bayes.Phsbin{1}(alltestidx{iter});
                        phsbintemp = phsbintemp(randperm(numel(phsbintemp)));%circshift(phsbintemp,30+randi(30));%
                        obj.Bayes.Phsbin{1}(alltestidx{iter}) = phsbintemp;
                    end
%                     for icell = 1:size(Xcell,2)
%                         nspk = unique(Ycell(Ycell(:,icell)>0,icell));
%                         for iter = 1:obj.Bayes.kfold
%                             for kspk = 1:numel(nspk)
%                                 idxAll = find(alltestidx{iter} & Ycell(:,icell)==nspk(kspk));
%                                 Xcelltemp = Xcell(idxAll,icell);
%                                 Xcelltemp = Xcelltemp(randperm(numel(Xcelltemp)));
%                                 Xcell(idxAll,icell) = Xcelltemp;
%                             end
%                         end
%                         for kspk = 1:numel(nspk)
%                             idxAll = find(~idxref & spdidx & Ycell(:,icell)==nspk(kspk));
%                             Xcelltemp = Xcell(idxAll,icell);
%                             Xcelltemp = Xcelltemp(randperm(numel(Xcelltemp)));
%                             Xcell(idxAll,icell) = Xcelltemp;
%                         end
...
%                     end
        ...
        if obj.Bayes.FshuffleSpdbins
            disp('Randomized spd bins');
            nspdbins = max(obj.Bayes.Spdbin);
            neyebins = max(obj.Bayes.Eyebin);
            for phs = 1:obj.Bayes.nthetaphsbins
                for eye = 1:neyebins
                    ...
                    for icell = 1:size(Xcell,2)
                        ...
                        for kspk = 1:numel(nspk)
                            idxAll = find(~idxref & phsidx & Ycell(:,icell)==nspk(kspk));
                            Xcelltemp = Xcell(idxAll,icell);
                            Xcelltemp = Xcelltemp(randperm(numel(Xcelltemp)));
                            Xcell(idxAll,icell) = Xcelltemp;
                        end
...
%                 phsval = obj.Bayes.LFPphase{1};
                ...
                idx = idxref & obj.Bayes.goodidx;
                
                obj.Bayes.X = zeros(numel(Xfilt),1);
                ...
                for spd = 1:nspdbins
                    for eye = 1:neyebins
                        ...
                        [obj.Bayes.decOder, pred, Xdec, Posterior, ~, ~] = obj.Bayes.decOder.trainDecoder(Xcell(idx & spdidx & phsidx,:), Xfilt(idx & spdidx & phsidx), Y{phs}(idx & spdidx & phsidx,:),Yfilt{phs}(idx & spdidx & phsidx,:), T{phs}(idx & spdidx & phsidx), Tfilt{phs}(idx & spdidx & phsidx), obj.Bayes.Xsmth_win);
                        obj.Bayes.X(idx & spdidx & phsidx,:) = Xdec(1:min(end,numel(pred)));
                        obj.Bayes.Posterior0{iprobe}(idx & spdidx & phsidx,:) = Posterior;
                        
                        [pred, Xdec, Posterior, ~, ~] = obj.Bayes.decOder.predictBayesDecoder(X(~idx & spdidx & phsidx,:), Yfilt{phs}(~idx & spdidx & phsidx,:), Tfilt{phs}(~idx & spdidx & phsidx), obj.Bayes.type);
...
%             obj.Bayes.Posterior1{iprobe} = obj.Bayes.Posterior0{iprobe};
            
%             Pprior = zeros(1,Xrange);
%             tidxref = obj.getSubsets(obj.Bayes.TrainContrast, obj.Bayes.TrainGain, obj.Bayes.TrainRoomlength, obj.Bayes.TrainOutcome,obj.Bayes.speed_th);
%             for i = 1:Xrange
%                 Pprior(i) = sum(obj.Bayes.Xsmth == i  & tidxref);
%             end
%             Pprior = Pprior/sum(tidxref);
%             obj.Bayes.Posterior0{iprobe} = obj.Bayes.Posterior0{iprobe}.*repmat(Pprior,[size(obj.Bayes.Posterior0{iprobe},1) 1]);
%             obj.Bayes.Posterior0{iprobe} = obj.Bayes.Posterior0{iprobe}./repmat(nansum(obj.Bayes.Posterior0{iprobe}(:,:),2),[1 Xrange]);
%             obj.Bayes.Posterior0{iprobe} = obj.Bayes.Posterior0{iprobe}/(1/Xrange);
            
%             for g = 1:numel(obj.SubsetVal.gain)
%                 for cont = 1:numel(obj.SubsetVal.contrast)
%                     for r = 1:numel(obj.SubsetVal.roomlength)
%                         for o = 1:numel(obj.SubsetVal.outcome)
%                             tidx = obj.getSubsets(cont,g,r,o,-inf);
%                             obj.Bayes.Posterior0{iprobe}(tidx,:) = obj.Bayes.Posterior0{iprobe}(tidx,:)./repmat(nanmean(obj.Bayes.Posterior0{iprobe}(tidx,:),1),[sum(tidx) 1]);
%                         end
%                     end
%                 end
%             end
%             tidx = ~obj.Subset.noBlanks;
%             obj.Bayes.Posterior0{iprobe}(tidx,:) = obj.Bayes.Posterior0{iprobe}(tidx,:)./repmat(nanmean(obj.Bayes.Posterior0{iprobe}(tidx,:),1),[sum(tidx) 1]);
            
            
            obj.Bayes.MaxDecodedPosition0{iprobe} = getCircularAverage(obj.Bayes.Posterior0{iprobe}',0,0.1,0.05);
            obj.Bayes.MeanDecodedPosition0{iprobe} = getCircularAverage(obj.Bayes.Posterior0{iprobe}',0,1);
            obj.Bayes.DecodingError0{iprobe} = circ_dist(2*pi/obj.Bayes.numBins*(obj.Bayes.MaxDecodedPosition0{iprobe}),2*pi/obj.Bayes.numBins*(obj.Bayes.Xsmth))*obj.Bayes.numBins/(2*pi);
            obj.Bayes.DecodingBias0{iprobe} = circ_dist(2*pi/obj.Bayes.numBins*(obj.Bayes.MeanDecodedPosition0{iprobe}),2*pi/obj.Bayes.numBins*(obj.Bayes.MaxDecodedPosition0{iprobe}))*obj.Bayes.numBins/(2*pi);
            
%             obj.Bayes.MaxDecodedPosition1{iprobe} = getCircularAverage(obj.Bayes.Posterior1{iprobe}',0,0.1,0.05);
%             obj.Bayes.MeanDecodedPosition1{iprobe} = getCircularAverage(obj.Bayes.Posterior1{iprobe}',0,1);
%             obj.Bayes.DecodingError1{iprobe} = circ_dist(2*pi/obj.Bayes.numBins*(obj.Bayes.MaxDecodedPosition1{iprobe}),2*pi/obj.Bayes.numBins*(obj.Bayes.Xsmth))*obj.Bayes.numBins/(2*pi);
%             obj.Bayes.DecodingBias1{iprobe} = circ_dist(2*pi/obj.Bayes.numBins*(obj.Bayes.MeanDecodedPosition1{iprobe}),2*pi/obj.Bayes.numBins*(obj.Bayes.MaxDecodedPosition1{iprobe}))*obj.Bayes.numBins/(2*pi);
...
%                 obj.Bayes.PosError1{iprobe,1}(obj.Bayes.Xsmth0{1} == i,:) = circshift(obj.Bayes.Posterior1{iprobe,1}(obj.Bayes.Xsmth0{1} == i,:),floor(Prange/2)-i,2);
            end
...
%         obj.Bayes.Posterior1{iprobe,1} = NaN(size(obj.Bayes.Posterior0{1}));
%         obj.Bayes.PosError1{iprobe,1} = NaN(size(obj.Bayes.PosError0{1}));
    end
end
end