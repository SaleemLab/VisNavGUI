...
tidx = tidx & obj.data.es.smthBallSpd > speed_th & ~isnan(obj.data.es.smthBallSpd) & ~isnan(obj.data.es.smthTrajSpd) & ~isnan(obj.data.es.gain);

if FnoBlanks && isfield(obj.data.es,'blanks')
    tidx = tidx & obj.Subset.noBlanks;%~(obj.data.es.blanks) & ~(obj.data.es.afterblanks);
    for tt = -30:30
        tidx = tidx & circshift(obj.Subset.noBlanks,tt);%~(obj.data.es.blanks) & ~(obj.data.es.afterblanks);
    end
end
