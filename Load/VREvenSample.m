...
if nargin<3
    ...
    if numel(es.screenTimes) < numel(es.screenTimes2)
        maxTime  = min(find((es.screenTimes2-max(es.screenTimes))>0))-1;
        if isempty(maxTime)
    ...
    elseif numel(es.screenTimes) > numel(es.screenTimes2)
        ...
        maxTime  = min(find((es.screenTimes-max(es.screenTimes2))>0))-1;
        es.screenTimes = es.screenTimes(1:maxTime);
    end
    ...
    disp(['best delay of ' num2str(delaymin*1000) 'ms achieved with shift of ' num2str(bestdelay-6)])
end
