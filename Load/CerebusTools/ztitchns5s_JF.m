...
chN = zeros(1,length(ns5files2unify));
for i = 1:length(ns5files2unify)
    ns5 = openNSx(ns5files2unify(i).fullFilename,'t:1:2');
    chN(i) = ns5.MetaTags.ChannelCount;    
end;
if numel(unique(chN)) == 1
    nchan = unique(chN);
else
...
for f = 1:length(ns5files2unify)
  ...
  disp(ns5files2unify(f).fullFilename);
  for ichan = 1:nchan
    ns5 = openNSx(ns5files2unify(f).fullFilename,['c:' num2str(ichan)]);
    if ichan == 1
        data = zeros(nchan,ns5.MetaTags.DataPoints);
    end
    data(ichan,:) = int16(ns5.Data(1,:));
  end;  
