function es = VRwrap(es,roomlength,wrapfactor,Frecenter)
% roomlength = round(max(es.traj));
newroomlength = floor(roomlength/wrapfactor);
newroomPercent = newroomlength;%floor(round(max(es.trajPercent))/wrapfactor);
if Frecenter
...
es.trajPercentunwrapped = mod(es.trajPercent - rewPos,newroomPercent*wrapfactor);
es.trajPercentunwrapped = es.trajPercentunwrapped/newroomPercent*100;
