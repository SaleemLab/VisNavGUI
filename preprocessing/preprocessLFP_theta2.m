...
%         try
        ...
        if ~Fskip
            ...
            LFPthetaPhase = [];
            LFPtheta = [];
            thetafrange  = [6 9];%[6 12];%[5 10];%
            nevSamplingRateInKHZ = 30;
            VRsamplingfrequency = 60;%200;%
            ...
            [es, spikeRate, VRdata, VRdata_o, chans] = getVRspikes(animal,iseries(s),iexp,1,100,1,0,shanknum,suffix,0,150,VRsamplingfrequency);
...
%             Timestamps.Chn = Timestamps.Chn - (screenTimes(1)./(exptInfo.downsample*nevSamplingRateInKHZ) - es.screenTimes2(2));
            Timestamps.Chn = Timestamps.Chn - (screenTimes(1)./(exptInfo.downsample*nevSamplingRateInKHZ) - es.screenTimes2(1));
...
%                 end
                chans(ichan).spiketimes = round(chans(ichan).spiketimes./binsize)+spkstart;
...
%                 theta = LFPsignals(double(pepNEV.ns.Data.data(chan_list(ch),:)), Timestamps.samplingRateOrig, 500, 10000);
%                 Chntemp = abs(theta.hill);
%                 Chntemp = decimate(Chntemp, Timestamps.samplingRateOrig/exptInfo.downsample);
...
%             end
            ...
            %theta from mua
            if sum(probeID == 1)>0
                AllspikeTrain = zeros(size(LFPshifted,1),1);
                for ichan = 1:length(chans)
                    if probeID(ichan) == 1 %muacellidx(ichan)
                        AllspikeTrain(chans(ichan).spiketimes) = AllspikeTrain(chans(ichan).spiketimes) + 1;
                    end
                end
                theta = LFPsignals(AllspikeTrain', 1000, thetafrange(1), thetafrange(2));
                theta = theta.hill';
                thetaphs0 = round(mod(360 + 180/pi*angle(theta),360));
                thetapeak_idx0 = find([0;abs(diff(mod(thetaphs0,360)))>180]);
                shorttheta_lim = 60;%ms
                ...
                samplingwin = round(min(diff(1000*es.sampleTimes)));
                theta_unwraped = smooth(theta_unwraped,samplingwin,'moving');
                theta_resample = interp1(Timestamps.Chn', theta_unwraped', es.sampleTimes);
                LFPthetaPhase(nbCh+1,:) = uint8(round(mod(360/(2*pi)*theta_resample,360)/2));%uint8(round(mod(360 - 180/pi*theta_resample,360)/2));
                LFPthetaPower(nbCh+1,:) = interp1(Timestamps.Chn', abs(theta)', es.sampleTimes);
                LFPtheta(nbCh+1,:) = interp1(Timestamps.Chn', real(theta)', es.sampleTimes);
            end
            
            if sum(probeID == 2)>0
                AllspikeTrain = zeros(size(LFPshifted,1),1);
                for ichan = 1:length(chans)
                    if probeID(ichan) == 2 %muacellidx(ichan)
                        AllspikeTrain(chans(ichan).spiketimes) = AllspikeTrain(chans(ichan).spiketimes) + 1;
                    end
                end
                theta = LFPsignals(AllspikeTrain', 1000, thetafrange(1), thetafrange(2));
                ...
                theta_unwraped = smooth(theta_unwraped,samplingwin,'moving');
                theta_resample = interp1(Timestamps.Chn', theta_unwraped', es.sampleTimes);
                LFPthetaPhase(nbCh+3,:) = uint8(round(mod(360/(2*pi)*theta_resample,360)/2));%uint8(round(mod(360 - 180/pi*theta_resample,360)/2));
                LFPthetaPower(nbCh+3,:) = interp1(Timestamps.Chn', abs(theta)', es.sampleTimes);
                LFPtheta(nbCh+3,:) = interp1(Timestamps.Chn', real(theta)', es.sampleTimes);
            end
%           
...
%             end
            ...
            if strcmp(Probe,'CA1')
                save([dDIRname filesep fname '_LFP6.mat'], 'LFPthetaPhase','LFPthetaPower','LFPtheta','corr_LFPmua','SpkthetaPhase','chref','corr_LFPmua2All','-v7.3');%,'LFP', 'LFPphase', 'theta',  'timestamps','-v7.3');
            else
                save([dDIRname filesep fname '_' Probe '_LFP6.mat'], 'LFPthetaPhase','LFPthetaPower','LFPtheta','corr_LFPmua','SpkthetaPhase','chref','corr_LFPmua2All','-v7.3');%,'LFP', 'LFPphase', 'theta',  'timestamps','-v7.3');
            end
        end
%         catch
