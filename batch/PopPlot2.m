        ...
        for ianimal = 1:nanimal
            ...
            for iseries = 1:size(meanErr,2)
                if ~isempty(meanErr{ianimal,iseries,iprobe,g}) && ~isnan(meanErr{ianimal,iseries,iprobe,g})
                    meanLickAll{iprobe,g} = [meanLickAll{iprobe,g} max([popres.s_medianlickX{ianimal,iseries,1,g} popres.s_medianlickX{ianimal,iseries,size(popres.s_meanlickX,3),g}])];%[meanLickAll{iprobe,g}  max([popres.s_meanlickX{ianimal,iseries,1,g} popres.s_meanlickX{ianimal,iseries,size(popres.s_meanlickX,3),g}])];
%                     if g~=2
...
%                     end
%                     if g~=2
%                         meanErrAll{iprobe,g} = [meanErrAll{iprobe,g}  Prange/(2*pi)*circ_mean(2*pi/Prange*(popres.s_ErrXpred{ianimal,iseries,iprobe,g}-Prange/2))];%meanErr{ianimal,iseries,iprobe,g}];
%                     else
%                         meanErrAll{iprobe,g} = [meanErrAll{iprobe,g}  0];
%                     end
                    ...
                    if isfield(popres,'ErrXpredPhsAllZscore')
                        thetaZscoreAll{iprobe,g} = [thetaZscoreAll{iprobe,g} popres.ErrXpredPhsAllZscore{ianimal,iseries,iprobe,g}];
                    end
...
%         ishift = 0;
%         for xshift = -Prange/2+1:Prange/2
%             ishift = ishift+1;
...
%         
        ...
        hold on;plot(X,Xdec{iprobe,g},'w');
        hold on;plot([0 Prange],[0 Prange],'w--');
        cmap = cmocean('amp');
        colormap(cmap);%colormap(hot);%colormap(parula);%colormap(RedWhiteBlue);%
...
%         hold on;plot(Xdec{iprobe,g},'k');
        ...
        if g~=2 || ~strcmp(errname,'Corr')
            hold on;ciplot(ax,X,Errdec{iprobe,g} - Prange/2,Errdecstd{iprobe,g},0.4,cl{g});
...
%         set(gca,'Xlim',[X(1)/gainVal(1) X(end)/gainVal(1)],'Ylim',[-20 20]);
        ...
        if iprobe == 1
            ...
            subplot(3,6,14);
            [visspeedpro,x,visspeedprostd] = fast1Dmap(popres.X{1,g},popres.visspeed{1,g},1,1,[], true);
            hold on;plot(x,visspeedpro,cl{g});
            hold on;ciplot(gca,x,visspeedpro,visspeedprostd,0.1,cl{g});
            xlabel('position');
            ylabel('running speed');
        else
            subplot(3,6,13);
...
if ~isempty(popresCorr)
    ...
    for g = [2 1 3]
        ...
        set(gca,'Xlim',[1 Prange],'Clim',[-0.4 0.4],'Ylim',[1 Prange],'Ydir','normal','XAxisLocation','origin','YAxisLocation','origin','PlotBoxAspectRatio', [1 1 1])
        hold on;plot([Prange/2 Prange/2],[1 Prange],'k');
        hold on;plot([1 Prange],[Prange/2 Prange/2],'k');
        hold on;plot([1 Prange],[1 Prange],'k');
        ...
        set(gca,'Xlim',[1 Prange],'Clim',[-0.05 0.05],'Ylim',[1 Prange],'Ydir','normal','XAxisLocation','origin','YAxisLocation','origin','PlotBoxAspectRatio', [1 1 1])
        hold on;plot([Prange/2 Prange/2],[1 Prange],'k');
        hold on;plot([1 Prange],[Prange/2 Prange/2],'k');
        hold on;plot([1 Prange],[1 Prange],'k');
...
nSec_blank = 6;
for iprobe = 1:nProbe
    subplot(nProbe,2,(iprobe-1)*2+1);
    imagesc(linspace(-nSec_blank/2,nSec_blank/2,size(popres.PostBlankOnAll{iprobe},2)),1:size(popres.PostBlankOnAll{iprobe},1),popres.PostBlankOnAll{iprobe});
    hold on;plot([0 0],[1 size(popres.PostBlankOnAll{iprobe},1)],'--k');
    set(gca,'Clim',Clim,'Ydir','normal','PlotBoxAspectRatio', [1 1 1]);
    subplot(nProbe,2,(iprobe-1)*2+2);
    imagesc(linspace(-nSec_blank/2,nSec_blank/2,size(popres.PostBlankOffAll{iprobe},2)),1:size(popres.PostBlankOffAll{iprobe},1),popres.PostBlankOffAll{iprobe});
    hold on;plot([0 0],[1 size(popres.PostBlankOnAll{iprobe},1)],'--k');
    set(gca,'Clim',Clim,'Ydir','normal','PlotBoxAspectRatio', [1 1 1]);
    cmap = cmocean('amp');
    colormap(cmap);
...
for g = [1 3]
    ...
    for xx = 1:size(meanErrAllSeg{iprobe,g},1)
        ...
        hold on;
        lickshift = meanLickAll{iprobe,g}-meanLickAll{iprobe,2};
        lickshift(lickshift<-50) = lickshift(lickshift<-50) + 100;
        lickshift(lickshift>50) = lickshift(lickshift>50) - 100;
        scatter(meanErrAll{iprobe,g}-meanErrAll{iprobe,2},lickshift,cl{g});
    ...
    subplot(3,5,1);
    hshift = histogram(lickshift,[-inf -10.5:1:10.5 inf],'EdgeColor','none','FaceColor',cl{g});
...
if isfield(popres,['s_' errnameTheta 'XPhsAll'])
    ...
    for iprobe = 1:nProbe
        for g = [2 1 3]
            ...
            map = popres.([errnameTheta 'XPhsAll']){iprobe,g};
            [xorig,yorig] = meshgrid(0.5:size(map,2)-0.5,0.5:size(map,1)-0.5);
            [xinterp,yinterp] = meshgrid(0.5:0.1:size(map,2)-0.5,0.5:0.1:size(map,1)-0.5);
            map = interp2(xorig,yorig,map,xinterp,yinterp);
            if g == 2
                imagesc(map);%./repmat(max(popres.PostErrPhsAll{iprobe,g},[],1),[size(popres.PostErrPhsAll{iprobe,g},1) 1]));
            end
            set(gca,'Clim',[0 1],'Ylim',[300 700],'Ydir','normal');
            subplot(2,4,(iprobe-1)*4+2);
            hold on;
            vec = popres.([errnameTheta 'XpredPhsAll']){iprobe,g};
            vec = [vec;vec(1)];
            vecSE = popres.([errnameTheta 'XpredPhsAll_SE']){iprobe,g};
            vecSE = [vecSE;vecSE(1)];
            if g == 2
                ciplot(gca,linspace(0,360,numel(vec)),vec-mean(vec),vecSE,0.5,cl{g});
            else
                plot(linspace(0,360,numel(vec)),vec-mean(vec),cl{g});
            end
            subplot(2,4,(iprobe-1)*4+3);
            hold on;
            vec = popres.([errnameTheta 'XpredNormPhsAll']){iprobe,g};
            vec = [vec;vec(1)];
            vecSE = popres.([errnameTheta 'XpredNormPhsAll_SE']){iprobe,g};
            vecSE = [vecSE;vecSE(1)];
            if g == 2
                ciplot(gca,linspace(0,360,numel(vec)),vec,vecSE,0.5,cl{g});
            else
                plot(gca,linspace(0,360,numel(vec)),vec,cl{g});
            end
