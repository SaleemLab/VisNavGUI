...
popres.s_FsmoothOutput = true;
popres.FsmoothOutput = false;
...
popres.PostBlankOnAll = cell(nProbe);
popres.BlankOnsum = cell(nProbe);
popres.PostBlankOffAll = cell(nProbe);
popres.BlankOffsum = cell(nProbe);
popres.s_PostBlankOnAll = cell(nProbe);
popres.s_BlankOnsum = cell(nProbe);
popres.s_PostBlankOffAll = cell(nProbe);
popres.s_BlankOffsum = cell(nProbe);
...
popres.s_LFPphase = cell(nProbe,ngain);
popres.s_meanlickX = cell(nProbe,ngain);
popres.s_medianlickX = cell(nProbe,ngain);
popres.s_correcttrials = cell(nProbe,ngain);
popres.s_incorrecttrials = cell(nProbe,ngain);
popres.performance = cell(nProbe,ngain);
for iprobe = 1:nProbe
    popres.PostBlankOnAll{iprobe} = 0;
    popres.BlankOnsum{iprobe} = 0;
    popres.PostBlankOffAll{iprobe} = 0;
    popres.BlankOffsum{iprobe} = 0;
    for g = 1:ngain
        popres.PostXAll{iprobe,g} = 0;
        popres.ErrXAll{iprobe,g} = 0;
        popres.BiasXAll{iprobe,g} = 0;
        popres.Xsum{iprobe,g} = 0;
        popres.s_PostBlankAll{iprobe,g} = cell(nProbe,ngain);
        popres.s_Blanksum{iprobe,g} = cell(nProbe,ngain);
        ...
        if isfield(res,'XSumPhsAll')
            for kiter = 1:size(res.XSumPhsAllCVO,5)
                ...
                popres.ErrXPhsAll2CV{iprobe,g,kiter} = 0;
        ...
        for ianimal = 1:nanimal
            for iseries0 = 1:numel(expt(ianimal).series)
                if ((iprobe == 1 && expt(ianimal).goodCA1{iseries0} == 1) || (iprobe == 2 && expt(ianimal).goodV1{iseries0} == 1))
                    if ~isempty(strfind(area_str,expt(ianimal).area{iseries0}))
                        if strcmp(popres.average_type,'animals')
                            iseries = 1;%iseries0;
                        else
                            iseries = iseries0;
                        end
                        ...
                        if isfield(res,'XSumPhsAll')
                            ...
                            popres.s_ErrXPhsAll2{ianimal,iseries,iprobe,g} = 0;
                        ...
                        if isfield(res,'PostXSum_blankOnset') && g==2
                            popres.s_PostBlankOnAll{ianimal,iseries,iprobe} = 0;
                            popres.s_BlankOnsum{ianimal,iseries,iprobe} = 0;
                            popres.s_PostBlankOffAll{ianimal,iseries,iprobe} = 0;
                            popres.s_BlankOffsum{ianimal,iseries,iprobe} = 0;
                        end
                        popres.s_X{ianimal,iseries,iprobe,g} = [];
                        popres.s_X_licks{ianimal,iseries,iprobe,g} = [];
                        ...
                        popres.s_LFPphase{ianimal,iseries,iprobe,g} = [];
                        popres.s_meanlickX{ianimal,iseries,iprobe,g} = [];
                        popres.s_medianlickX{ianimal,iseries,iprobe,g} = [];
                        ...
                        if isfield(res,'XSumPhsAll')
                            for kiter = 1:size(res.XSumPhsAllCVO,5)
                                ...
                                popres.s_ErrXPhsAll2CV{ianimal,iseries,iprobe,g,kiter} = 0;
    ...
    for ianimal = 1:nanimal
        ...
        for iseries = 1:numel(expt(ianimal).series)
            if ((iprobe == 1 && expt(ianimal).goodCA1{iseries} == 1) || (iprobe == 2 && expt(ianimal).goodV1{iseries} == 1))
                if ~isempty(strfind(area_str,expt(ianimal).area{iseries}))
                    if strcmp(popres.average_type,'animals')
                        iseries_ave = 1;%iseries;
                    else
                        iseries_ave = iseries;
                    end
                    Prange = size(res.([resErrName]){ianimal,iseries,iprobe,2},1);
                    Xrange = size(res.([resErrName]){ianimal,iseries,iprobe,2},2);
                    for g = [2 1 3]
                        if ((iprobe == 1 && expt(ianimal).goodCA1dec{g}{iseries} == 1) || (iprobe == 2 && expt(ianimal).goodV1dec{g}{iseries} == 1))
                            if ~isempty(res.XSum{ianimal,iseries,iprobe,g})
                                ...
                                tidx_licks = res.tidx_licks{ianimal,iseries,iprobe,g};
                                ...
                                popres.licks{iprobe,g} = [popres.licks{iprobe,g} ; res.firstgoodlick{ianimal,iseries}(tidx_licks)];
                                popres.X_licks{iprobe,g} = [popres.X_licks{iprobe,g} ; res.X{ianimal,iseries}(tidx_licks)];
                                popres.X{iprobe,g} = [popres.X{iprobe,g} ; res.X{ianimal,iseries}(tidx)];
                                popres.LFPphase{iprobe,g} = [popres.LFPphase{iprobe,g} ; res.BayesLFPphase{ianimal,iseries,iprobe}(tidx)];
                                
                                if strcmp(popres.average_type,'trials')
                                    ...
                                    if isfield(res,'PostXSum_blankOnset') && g==2
                                        popres.PostBlankOnAll{iprobe} = popres.PostBlankOnAll{iprobe} + res.PostXSum_blankOnset{ianimal,iseries,iprobe};
                                        popres.BlankOnsum{iprobe} = popres.BlankOnsum{iprobe} + res.XSum_blankOnset{ianimal,iseries,iprobe};
                                        popres.PostBlankOffAll{iprobe} = popres.PostBlankOffAll{iprobe} + res.PostXSum_blankOffset{ianimal,iseries,iprobe};
                                        popres.BlankOffsum{iprobe} = popres.BlankOffsum{iprobe} + res.XSum_blankOffset{ianimal,iseries,iprobe};
                                    end
...
%                                                 ./res.XSumPhsAll{ianimal,iseries,iprobe,g},3));
                                            ...
                                            popres.ErrXPhsAll{iprobe,g} = popres.ErrXPhsAll{iprobe,g} + squeeze(nansum(res.([resErrName 'PhsAll']){ianimal,iseries,iprobe,g},3));
                                            popres.ErrXPhsAll2{iprobe,g} = popres.ErrXPhsAll2{iprobe,g} + squeeze(nansum(res.([resErrName 'PhsAll2']){ianimal,iseries,iprobe,g},3));
                                            popres.BiasXPhsAll{iprobe,g} = popres.BiasXPhsAll{iprobe,g} + squeeze(nansum(res.([resBiasName 'PhsAll']){ianimal,iseries,iprobe,g},3));
                                        end
%                                         popres.XsumPhsAll{iprobe,g} = popres.XsumPhsAll{iprobe,g} + ones(size(res.XSumPhsAll{ianimal,iseries,iprobe,g},1),size(res.XSumPhsAll{ianimal,iseries,iprobe,g},2));
                                        popres.XsumPhsAll{iprobe,g} = popres.XsumPhsAll{iprobe,g} + squeeze(nansum(res.XSumPhsAll{ianimal,iseries,iprobe,g},3));  
...
%                                                     ./res.XSumPhsAllCVO{ianimal,iseries,iprobe,g,kiter},3));
                                                popres.ErrXPhsAllCV{iprobe,g,kiter} = popres.ErrXPhsAllCV{iprobe,g,kiter} + squeeze(nansum(res.([resErrName 'PhsAllCVO']){ianimal,iseries,iprobe,g,kiter},3));
                                                popres.ErrXPhsAll2CV{iprobe,g,kiter} = popres.ErrXPhsAll2CV{iprobe,g,kiter} + squeeze(nansum(res.([resErrName 'PhsAll2CVO']){ianimal,iseries,iprobe,g,kiter},3));
                                                popres.BiasXPhsAllCV{iprobe,g,kiter} = popres.BiasXPhsAllCV{iprobe,g,kiter} + squeeze(nansum(res.([resBiasName 'PhsAllCVO']){ianimal,iseries,iprobe,g,kiter},3));
                                            end
%                                             popres.XsumPhsAllCV{iprobe,g,kiter} = popres.XsumPhsAllCV{iprobe,g,kiter} + ones(size(res.XSumPhsAllCVO{ianimal,iseries,iprobe,g,kiter},1),size(res.XSumPhsAllCVO{ianimal,iseries,iprobe,g,kiter},2));
                                            popres.XsumPhsAllCV{iprobe,g,kiter} = popres.XsumPhsAllCV{iprobe,g,kiter} + squeeze(nansum(res.XSumPhsAllCVO{ianimal,iseries,iprobe,g,kiter},3));
                                        end
                                    end
                                end
                                
                                popres.s_runspeed{ianimal,iseries_ave,iprobe,g} = [popres.s_runspeed{ianimal,iseries_ave,iprobe,g} ; res.runSpeed{ianimal,iseries}(tidx)];
                                popres.s_visspeed{ianimal,iseries_ave,iprobe,g} = [popres.s_visspeed{ianimal,iseries_ave,iprobe,g} ; res.visSpeed{ianimal,iseries}(tidx)];
                                popres.s_licks{ianimal,iseries_ave,iprobe,g} = [popres.s_licks{ianimal,iseries_ave,iprobe,g} ; res.firstgoodlick{ianimal,iseries}(tidx_licks)];
                                popres.s_LFPphase{ianimal,iseries_ave,iprobe,g} = [popres.s_LFPphase{ianimal,iseries_ave,iprobe,g} ; res.BayesLFPphase{ianimal,iseries,iprobe}(tidx)];
                                
                                popres.s_X{ianimal,iseries_ave,iprobe,g} = [popres.s_X{ianimal,iseries_ave,iprobe,g} ; res.X{ianimal,iseries}(tidx)];
                                popres.s_X_licks{ianimal,iseries_ave,iprobe,g} = [popres.s_X_licks{ianimal,iseries_ave,iprobe,g} ; res.X{ianimal,iseries}(tidx_licks)];
                                popres.s_PostXAll{ianimal,iseries_ave,iprobe,g} = popres.s_PostXAll{ianimal,iseries_ave,iprobe,g} + res.(resPosName){ianimal,iseries,iprobe,g};
                                popres.s_ErrXAll{ianimal,iseries_ave,iprobe,g} = popres.s_ErrXAll{ianimal,iseries_ave,iprobe,g} + res.(resErrName){ianimal,iseries,iprobe,g};
                                popres.s_BiasXAll{ianimal,iseries_ave,iprobe,g} = popres.s_BiasXAll{ianimal,iseries_ave,iprobe,g} + res.(resBiasName){ianimal,iseries,iprobe,g};
                                popres.s_Xsum{ianimal,iseries_ave,iprobe,g} = popres.s_Xsum{ianimal,iseries_ave,iprobe,g} + res.XSum{ianimal,iseries,iprobe,g};
                                
                                if isfield(res,'PostXSum_blankOnset') && g==2
                                    popres.s_PostBlankOnAll{ianimal,iseries_ave,iprobe} = popres.s_PostBlankOnAll{ianimal,iseries_ave,iprobe} + res.PostXSum_blankOnset{ianimal,iseries,iprobe};
                                    popres.s_BlankOnsum{ianimal,iseries_ave,iprobe} = popres.s_BlankOnsum{ianimal,iseries_ave,iprobe} + res.XSum_blankOnset{ianimal,iseries,iprobe};
                                    popres.s_PostBlankOffAll{ianimal,iseries_ave,iprobe} = popres.s_PostBlankOffAll{ianimal,iseries_ave,iprobe} + res.PostXSum_blankOffset{ianimal,iseries,iprobe};
                                    popres.s_BlankOffsum{ianimal,iseries_ave,iprobe} = popres.s_BlankOffsum{ianimal,iseries_ave,iprobe} + res.XSum_blankOffset{ianimal,iseries,iprobe};
                                end
                                
                                nfold = size(res.XSumCVO,5);
                                for kiter = 1:nfold
                                    popres.s_PostXAllCV{ianimal,iseries_ave,iprobe,g,kiter} = popres.s_PostXAllCV{ianimal,iseries_ave,iprobe,g,kiter} + res.([resPosName 'CVO']){ianimal,iseries,iprobe,g,kiter};
                                    popres.s_ErrXAllCV{ianimal,iseries_ave,iprobe,g,kiter} = popres.s_ErrXAllCV{ianimal,iseries_ave,iprobe,g,kiter} + res.([resErrName 'CVO']){ianimal,iseries,iprobe,g,kiter};
                                    popres.s_BiasXAllCV{ianimal,iseries_ave,iprobe,g,kiter} = popres.s_BiasXAllCV{ianimal,iseries_ave,iprobe,g,kiter} + res.([resBiasName 'CVO']){ianimal,iseries,iprobe,g,kiter};
                                    popres.s_XsumCV{ianimal,iseries_ave,iprobe,g,kiter} = popres.s_XsumCV{ianimal,iseries_ave,iprobe,g,kiter} + res.XSumCVO{ianimal,iseries,iprobe,g,kiter};
                                end
                                
                                popres.s_PostXAllSpd{ianimal,iseries_ave,iprobe,g} = popres.s_PostXAllSpd{ianimal,iseries_ave,iprobe,g} + res.([resPosName 'Spd']){ianimal,iseries,iprobe,g};
                                popres.s_ErrXAllSpd{ianimal,iseries_ave,iprobe,g} = popres.s_ErrXAllSpd{ianimal,iseries_ave,iprobe,g} + res.([resErrName 'Spd']){ianimal,iseries,iprobe,g};
                                popres.s_BiasXAllSpd{ianimal,iseries_ave,iprobe,g} = popres.s_BiasXAllSpd{ianimal,iseries_ave,iprobe,g} + res.([resBiasName 'Spd']){ianimal,iseries,iprobe,g};
                                popres.s_XsumSpd{ianimal,iseries_ave,iprobe,g} = popres.s_XsumSpd{ianimal,iseries_ave,iprobe,g} + res.XSumSpd{ianimal,iseries,iprobe,g};
                                
                                popres.s_PostXAllEye{ianimal,iseries_ave,iprobe,g} = popres.s_PostXAllEye{ianimal,iseries_ave,iprobe,g} + res.([resPosName 'Eye']){ianimal,iseries,iprobe,g};
                                popres.s_ErrXAllEye{ianimal,iseries_ave,iprobe,g} = popres.s_ErrXAllEye{ianimal,iseries_ave,iprobe,g} + res.([resErrName 'Eye']){ianimal,iseries,iprobe,g};
                                popres.s_BiasXAllEye{ianimal,iseries_ave,iprobe,g} = popres.s_BiasXAllEye{ianimal,iseries_ave,iprobe,g} + res.([resBiasName 'Eye']){ianimal,iseries,iprobe,g};
                                popres.s_XsumEye{ianimal,iseries_ave,iprobe,g} = popres.s_XsumEye{ianimal,iseries_ave,iprobe,g} + res.XSumEye{ianimal,iseries,iprobe,g};
                                
                                popres.s_PostXAllVis{ianimal,iseries_ave,iprobe,g} = popres.s_PostXAllVis{ianimal,iseries_ave,iprobe,g} + res.([resPosName 'Vis']){ianimal,iseries,iprobe,g};
                                popres.s_ErrXAllVis{ianimal,iseries_ave,iprobe,g} = popres.s_ErrXAllVis{ianimal,iseries_ave,iprobe,g} + res.([resErrName 'Vis']){ianimal,iseries,iprobe,g};
                                popres.s_BiasXAllVis{ianimal,iseries_ave,iprobe,g} = popres.s_BiasXAllVis{ianimal,iseries_ave,iprobe,g} + res.([resBiasName 'Vis']){ianimal,iseries,iprobe,g};
                                popres.s_XsumVis{ianimal,iseries_ave,iprobe,g} = popres.s_XsumVis{ianimal,iseries_ave,iprobe,g} + res.XSumVis{ianimal,iseries,iprobe,g};
...
%                                     popres.s_XsumPhsAll{ianimal,iseries,iprobe,g} = popres.s_XsumPhsAll{ianimal,iseries,iprobe,g} + res.XSumPhsAll{ianimal,iseries,iprobe,g};
                                    Phsrange = size(res.([resErrName 'PhsAll']){ianimal,iseries,iprobe,g},2);
                                    if strcmp(popres.DecAveType,'DecPost') || strcmp(popres.DecAveType,'DecError')
%                                         popres.s_ErrXPhsAll{ianimal,iseries_ave,iprobe,g} = popres.s_ErrXPhsAll{ianimal,iseries_ave,iprobe,g} + squeeze(nanmean(res.([resErrName 'PhsAll']){ianimal,iseries,iprobe,g}...
%                                                                                                                                                         ./res.XSumPhsAll{ianimal,iseries,iprobe,g},3));
%                                         popres.s_BiasXPhsAll{ianimal,iseries_ave,iprobe,g} = popres.s_BiasXPhsAll{ianimal,iseries_ave,iprobe,g} + squeeze(nanmean(res.([resBiasName 'PhsAll']){ianimal,iseries,iprobe,g}...
%                                                                                                                                                           ./res.XSumPhsAll{ianimal,iseries,iprobe,g},3));
                                        popres.s_ErrXPhsAll{ianimal,iseries_ave,iprobe,g} = popres.s_ErrXPhsAll{ianimal,iseries_ave,iprobe,g} + squeeze(nansum(res.([resErrName 'PhsAll']){ianimal,iseries,iprobe,g},3));
                                        popres.s_ErrXPhsAll2{ianimal,iseries_ave,iprobe,g} = popres.s_ErrXPhsAll2{ianimal,iseries_ave,iprobe,g} + squeeze(nansum(res.([resErrName 'PhsAll2']){ianimal,iseries,iprobe,g},3));
                                        popres.s_BiasXPhsAll{ianimal,iseries_ave,iprobe,g} = popres.s_BiasXPhsAll{ianimal,iseries_ave,iprobe,g} + squeeze(nansum(res.([resBiasName 'PhsAll']){ianimal,iseries,iprobe,g},3));
                                    end
%                                     popres.s_XsumPhsAll{ianimal,iseries_ave,iprobe,g} = popres.s_XsumPhsAll{ianimal,iseries_ave,iprobe,g} + ones(size(res.XSumPhsAll{ianimal,iseries,iprobe,g},1),size(res.XSumPhsAll{ianimal,iseries,iprobe,g},2));
                                    popres.s_XsumPhsAll{ianimal,iseries_ave,iprobe,g} = popres.s_XsumPhsAll{ianimal,iseries_ave,iprobe,g} + squeeze(nansum(res.XSumPhsAll{ianimal,iseries,iprobe,g},3));
                                    
                                    for ispd = 1:res.nSpdPhsbins
                                        popres.s_ErrXAllPhs{ianimal,iseries_ave,iprobe,ispd,g} = popres.s_ErrXAllPhs{ianimal,iseries_ave,iprobe,ispd,g} + res.([resErrName 'Phs']){ianimal,iseries,iprobe,ispd,g};
                                        popres.s_BiasXAllPhs{ianimal,iseries_ave,iprobe,ispd,g} = popres.s_BiasXAllPhs{ianimal,iseries_ave,iprobe,ispd,g} + res.([resBiasName 'Phs']){ianimal,iseries,iprobe,ispd,g};
                                        popres.s_XsumPhs{ianimal,iseries_ave,iprobe,ispd,g} = popres.s_XsumPhs{ianimal,iseries_ave,iprobe,ispd,g} + res.XSumPhs{ianimal,iseries,iprobe,ispd,g};
                                    end
...
%                                         popres.s_XsumPhsAllCV{ianimal,iseries,iprobe,g,kiter} = popres.s_XsumPhsAllCV{ianimal,iseries,iprobe,g,kiter} + res.XSumPhsAllCVO{ianimal,iseries,iprobe,g,kiter};
                                        if strcmp(popres.DecAveType,'DecPost') || strcmp(popres.DecAveType,'DecError')
%                                             popres.s_ErrXPhsAllCV{ianimal,iseries_ave,iprobe,g,kiter} = popres.s_ErrXPhsAllCV{ianimal,iseries_ave,iprobe,g,kiter} + squeeze(nanmean(res.([resErrName 'PhsAllCVO']){ianimal,iseries,iprobe,g,kiter}...
%                                                                                                                                                                             ./res.XSumPhsAllCVO{ianimal,iseries,iprobe,g,kiter},3));
%                                             popres.s_BiasXPhsAllCV{ianimal,iseries_ave,iprobe,g,kiter} = popres.s_BiasXPhsAllCV{ianimal,iseries_ave,iprobe,g,kiter} + squeeze(nanmean(res.([resBiasName 'PhsAllCVO']){ianimal,iseries,iprobe,g,kiter}...
%                                                                                                                                                                               ./res.XSumPhsAllCVO{ianimal,iseries,iprobe,g,kiter},3));
                                            popres.s_ErrXPhsAllCV{ianimal,iseries_ave,iprobe,g,kiter} = popres.s_ErrXPhsAllCV{ianimal,iseries_ave,iprobe,g,kiter} + squeeze(nansum(res.([resErrName 'PhsAllCVO']){ianimal,iseries,iprobe,g,kiter},3));
                                            popres.s_ErrXPhsAll2CV{ianimal,iseries_ave,iprobe,g,kiter} = popres.s_ErrXPhsAll2CV{ianimal,iseries_ave,iprobe,g,kiter} + squeeze(nansum(res.([resErrName 'PhsAll2CVO']){ianimal,iseries,iprobe,g,kiter},3));
                                            popres.s_BiasXPhsAllCV{ianimal,iseries_ave,iprobe,g,kiter} = popres.s_BiasXPhsAllCV{ianimal,iseries_ave,iprobe,g,kiter} + squeeze(nansum(res.([resBiasName 'PhsAllCVO']){ianimal,iseries,iprobe,g,kiter},3));
                                        end
%                                         popres.s_XsumPhsAllCV{ianimal,iseries_ave,iprobe,g,kiter} = popres.s_XsumPhsAllCV{ianimal,iseries_ave,iprobe,g,kiter} + ones(size(res.XSumPhsAllCVO{ianimal,iseries,iprobe,g,kiter},1),size(res.XSumPhsAllCVO{ianimal,iseries,iprobe,g,kiter},2));
                                        popres.s_XsumPhsAllCV{ianimal,iseries_ave,iprobe,g,kiter} = popres.s_XsumPhsAllCV{ianimal,iseries_ave,iprobe,g,kiter} + squeeze(nansum(res.XSumPhsAllCVO{ianimal,iseries,iprobe,g,kiter},3));
                                    end
                                end
                                popres.s_trialID{ianimal,iseries_ave,iprobe,g} = [popres.s_trialID{ianimal,iseries_ave,iprobe,g} ; res.trialID{ianimal,iseries}(tidx)];
                                ...
                                popres.s_correcttrials{ianimal,iseries_ave,iprobe,g} = [popres.s_correcttrials{ianimal,iseries_ave,iprobe,g} ; unique(res.trialID{ianimal,iseries}(res.tidx{ianimal,iseries,iprobe,g}))];
                                popres.s_incorrecttrials{ianimal,iseries_ave,iprobe,g} = [popres.s_incorrecttrials{ianimal,iseries_ave,iprobe,g} ; unique(res.trialID{ianimal,iseries}(tidx_incorrect))];
                            end
                        end
                    end
                end
        ...
        iseries_valid = 0;
        for iseries = 1:size(popres.s_X,2)
            for g = [2 1 3]
                if sum(popres.s_Xsum{ianimal,iseries,iprobe,g}) ~= 0                    
                    %transfer the performance in Batch
                    %BayesGrandAverage
                    ...
                    dxlick = 1;
                    Xsmthlick = Prange/popres.Xsmthwin;
                    maxtol_licks = 0.1;
                    amp_th_licks = 0;
                    [lickmap,x,~] = fast1Dmap(popres.s_X_licks{ianimal,iseries,iprobe,g},popres.s_licks{ianimal,iseries,iprobe,g},dxlick,1,Xsmthlick,1);
                    [lickmapref,x,~] = fast1Dmap(popres.s_X_licks{ianimal,iseries,iprobe,2},popres.s_licks{ianimal,iseries,iprobe,2},dxlick,1,Xsmthlick,1);
                    if ~isempty(lickmap) && ~isempty(lickmapref)
                        ...
                        lickcdf = cumsum(circshift(lickmap,-floor(numel(lickmap)/2)))/sum(lickmap);
                        popres.s_medianlickX{ianimal,iseries,iprobe,g} = find(lickcdf>=0.5,1,'first');
                        popres.s_medianlickX{ianimal,iseries,iprobe,g} = getCircularAverage(lickmap(:),0,1);
                    else
                        popres.s_meanlickX{ianimal,iseries,iprobe,g} = NaN;
                        popres.s_medianlickX{ianimal,iseries,iprobe,g} = NaN;
                    end
                    ...
                    if numel(popres.s_Xsum{ianimal,iseries,iprobe,g}(:)) > 1
                        ...
                        if isfield(res,'PostXSum_blankOnset') && g==2
                            [popres.s_PostBlankOnAll{ianimal,iseries,iprobe},~,~,~,~,~,~,~]...
                            = ComputePostAve(popres.s_PostBlankOnAll{ianimal,iseries,iprobe}, popres.s_BlankOnsum{ianimal,iseries,iprobe}, popres, popres.s_FsmoothOutput);
                            [popres.s_PostBlankOffAll{ianimal,iseries,iprobe},~,~,~,~,~,~,~]...
                            = ComputePostAve(popres.s_PostBlankOffAll{ianimal,iseries,iprobe}, popres.s_BlankOffsum{ianimal,iseries,iprobe}, popres, popres.s_FsmoothOutput);
                        end
                        ...
                        if isfield(res,'XSumPhsAll')
                            ...
                            [popres.s_ErrXPhsAll2{ianimal,iseries,iprobe,g},~,...
                                popres.s_ErrXpredPhsAll2{ianimal,iseries,iprobe,g},...
                                ~,...
                                ~,~,~,~]...
                                = ComputePostAve(popres.s_ErrXPhsAll2{ianimal,iseries,iprobe,g}, popres.s_XsumPhsAll{ianimal,iseries,iprobe,g}, popres, popres.s_FsmoothOutput, [], phsSmthWin, phsSmthWin, true, true);% NaN, NaN, true, true);
                            ...
                            if nfold > 1
                                ...
                                for kiter = 1:nfold
                                    ...
                                    [s_ErrXPhsAll2_iter,~,...
                                        s_ErrXpredPhsAll2_iter,~,~,~]...
                                        = ComputePostAve(popres.s_ErrXPhsAll2CV{ianimal,iseries,iprobe,g,kiter}, popres.s_XsumPhsAllCV{ianimal,iseries,iprobe,g,kiter}, popres, popres.s_FsmoothOutput, [], phsSmthWin, phsSmthWin, true, true);%NaN, NaN, true, true);%
                                    s_ErrXpredPhsAll2_iter = s_ErrXpredPhsAll2_iter - mean(s_ErrXpredPhsAll2_iter);
                                    
                                    s_ErrXPhsAll2_std = s_ErrXPhsAll2_std + (nfold - 1)/nfold*(s_ErrXPhsAll2_iter - popres.s_ErrXPhsAll2{ianimal,iseries,iprobe,g}).^2;
                                    s_ErrXpredPhsAll2_std = s_ErrXpredPhsAll2_std + (nfold - 1)/nfold*(Prange/(2*pi)*circ_dist(2*pi/Prange*s_ErrXpredPhsAll2_iter,2*pi/Prange*(popres.s_ErrXpredPhsAll2{ianimal,iseries,iprobe,g}-mean(popres.s_ErrXpredPhsAll{ianimal,iseries,iprobe,g})))).^2;
                                ...
                                popres.s_ErrXPhsAll2_SE{ianimal,iseries,iprobe,g} = sqrt(s_ErrXPhsAll2_std);
                                popres.s_ErrXpredPhsAll2_SE{ianimal,iseries,iprobe,g} = sqrt(s_ErrXpredPhsAll2_std);
                            ...
                            [~, imaxtheta] = max(abs(popres.s_ErrXpredPhsAll{ianimal,iseries,iprobe,g}-mean(popres.s_ErrXpredPhsAll{ianimal,iseries,iprobe,g})));
                            popres.ErrXpredPhsAllZscore{ianimal,iseries,iprobe,g} = abs((popres.s_ErrXpredPhsAll{ianimal,iseries,iprobe,g}(imaxtheta)-mean(popres.s_ErrXpredPhsAll{ianimal,iseries,iprobe,g}))/popres.s_ErrXpredPhsAll_SE{ianimal,iseries,iprobe,g}(imaxtheta));
                            [~, imaxtheta] = max(abs(popres.s_ErrXpredNormPhsAll{ianimal,iseries,iprobe,g}-mean(popres.s_ErrXpredNormPhsAll{ianimal,iseries,iprobe,g})));
                            popres.ErrXpredNormPhsAllZscore{ianimal,iseries,iprobe,g} = abs((popres.s_ErrXpredNormPhsAll{ianimal,iseries,iprobe,g}(imaxtheta)-mean(popres.s_ErrXpredNormPhsAll{ianimal,iseries,iprobe,g}))/popres.s_ErrXpredNormPhsAll_SE{ianimal,iseries,iprobe,g}(imaxtheta));
                        end
    ...
    disp(['Now averaging across ' popres.average_type]);
    switch popres.average_type
        case {'sessions','animals'}
            for g = [2 1 3]
                popref = [];
                if g ~= 2
                ...
                if isfield(res,'XSumPhsAll')
                    ...
                    [popres.ErrXPhsAll{iprobe,g}, popres.ErrXPhsAll2{iprobe,g}, popres.BiasXPhsAll{iprobe,g},...
                        ...
                        = ComputePostGrandAve(popres.s_ErrXPhsAll(:,:,iprobe,g),popres.s_ErrXPhsAll2(:,:,iprobe,g),popres.s_BiasXPhsAll(:,:,iprobe,g),popres.s_CorrXPhsAll(:,:,iprobe,g),popref,popres, true, false);
            ...
            for g = [2 1 3]
                if numel(popres.Xsum{iprobe,g}(:)) > 1
                    ...
                    [popres.PostXAll{iprobe,g},~,popres.PostXpred{iprobe,g},~,~,~,~,~]...
                        = ComputePostAve(popres.PostXAll{iprobe,g}, popres.Xsum{iprobe,g}, popres, true);
                    
                    if isfield(res,'PostXSum_blankOnset') && g==2
                        [popres.PostBlankOnAll{iprobe},~,~,~,~,~,~,~]...
                            = ComputePostAve(popres.PostBlankOnAll{iprobe}, popres.BlankOnsum{iprobe}, popres, true);
                        [popres.PostBlankOffAll{iprobe},~,~,~,~,~,~,~]...
                            = ComputePostAve(popres.PostBlankOffAll{iprobe}, popres.BlankOffsum{iprobe}, popres, true);
                    end
                    ...
                    [popres.ErrXAll{iprobe,g},...
                        ...
                        = ComputePostAve(popres.ErrXAll{iprobe,g}, popres.Xsum{iprobe,g}, popres, true, popref);
                    ...
                    [popres.BiasXAll{iprobe,g},...
                        ...
                        = ComputePostAve(popres.BiasXAll{iprobe,g}, popres.Xsum{iprobe,g}, popres, true, popref);
                    ...
                    if nfold > 1
                        ...
                        for kiter = 1:nfold
                            [PostXAll_iter,~,PostXpred_iter,~,~,~,~,~]...
                                = ComputePostAve(popres.PostXAllCV{iprobe,g,kiter}, popres.XsumCV{iprobe,g,kiter}, popres, true);
                            ...
                            [PostErrAll_iter,...
                                ...
                                = ComputePostAve(popres.ErrXAllCV{iprobe,g,kiter}, popres.XsumCV{iprobe,g,kiter}, popres, true, popref);
                            ...
                            [PostBiasAll_iter,...
                                ...
                                = ComputePostAve(popres.BiasXAllCV{iprobe,g}, popres.XsumCV{iprobe,g}, popres, true, popref);
                    ...
                    [popres.PostXAllSpd{iprobe,g},~,popres.PostXpredSpd{iprobe,g},~,~,~,~,~]...
                        = ComputePostAve(popres.PostXAllSpd{iprobe,g}, popres.XsumSpd{iprobe,g}, popres, true);
                    ...
                    [popres.ErrXAllSpd{iprobe,g},...
                        ...
                        = ComputePostAve(popres.ErrXAllSpd{iprobe,g}, popres.XsumSpd{iprobe,g}, popres, true, popref);
                    ...
                    [popres.BiasXAllSpd{iprobe,g},...
                        ...
                        = ComputePostAve(popres.BiasXAllSpd{iprobe,g}, popres.XsumSpd{iprobe,g}, popres, true, popref);
                    ...
                    [popres.PostXAllEye{iprobe,g},~,popres.PostXpredEye{iprobe,g},~,~,~,~,~]...
                        = ComputePostAve(popres.PostXAllEye{iprobe,g}, popres.XsumEye{iprobe,g}, popres, true);
                    ...
                    [popres.ErrXAllEye{iprobe,g},...
                        ...
                        = ComputePostAve(popres.ErrXAllEye{iprobe,g}, popres.XsumEye{iprobe,g}, popres, true, popref);
                    ...
                    [popres.BiasXAllEye{iprobe,g},...
                        ...
                        = ComputePostAve(popres.BiasXAllEye{iprobe,g}, popres.XsumEye{iprobe,g}, popres, true, popref);
                    ...
                    [popres.PostXAllVis{iprobe,g},~,popres.PostXpredVis{iprobe,g},~,~,~,~,~]...
                        = ComputePostAve(popres.PostXAllVis{iprobe,g}, popres.XsumVis{iprobe,g}, popres, true);
                    ...
                    [popres.ErrXAllVis{iprobe,g},...
                        ...
                        = ComputePostAve(popres.ErrXAllVis{iprobe,g}, popres.XsumVis{iprobe,g}, popres, true, popref);
                    ...
                    [popres.BiasXAllVis{iprobe,g},...
                        ...
                        = ComputePostAve(popres.BiasXAllVis{iprobe,g}, popres.XsumVis{iprobe,g}, popres, true, popref);
...
%                             phsSmthWin = NaN;
                        ...
                        for ispd = 1:res.nSpdPhsbins
                            [popres.ErrXAllPhs{iprobe,ispd,g},~,...
                                ...
                                = ComputePostAve(popres.ErrXAllPhs{iprobe,ispd,g}, popres.XsumPhs{iprobe,ispd,g}, popres, true, [], 2, 2, true, true);
                            
                            [popres.BiasXAllPhs{iprobe,ispd,g},~,...
                                popres.BiasXpredAllPhs{iprobe,ispd,g},~,~,~,~,~]...
                                = ComputePostAve(popres.BiasXAllPhs{iprobe,ispd,g}, popres.XsumPhs{iprobe,ispd,g}, popres, true, [], 2, 2, true, true);
                        end
                        ...
                        [popres.ErrXPhsAll{iprobe,g},~,...
                            ...
                            = ComputePostAve(popres.ErrXPhsAll{iprobe,g}, popres.XsumPhsAll{iprobe,g}, popres, true, [], phsSmthWin, phsSmthWin, true, true);% NaN, NaN, true, true);
                        
                        [popres.ErrXPhsAll2{iprobe,g},~,...
                            popres.ErrXpredPhsAll2{iprobe,g},...
                            ~,...
                            ~,~,~,~]...
                            = ComputePostAve(popres.ErrXPhsAll2{iprobe,g}, popres.XsumPhsAll{iprobe,g}, popres, true, [], phsSmthWin, phsSmthWin, true, true);% NaN, NaN, true, true);
                        
                        [popres.BiasXPhsAll{iprobe,g},~,...
                            popres.BiasXpredPhsAll{iprobe,g},~,~,~,~,~]...
                            = ComputePostAve(popres.BiasXPhsAll{iprobe,g}, popres.XsumPhsAll{iprobe,g}, popres, true, [], phsSmthWin, phsSmthWin, true, true);% NaN, NaN, true, true);
                        ...
                        if nfold > 1
                            ...
                            PostErrPhsAll2_std = 0;
                            PostErrpredPhsAll2_std = 0;
                            ...
                            for kiter = 1:nfold
                                [PostErrPhsAll_iter,~,...
                                    ...
                                    = ComputePostAve(popres.ErrXPhsAllCV{iprobe,g,kiter}, popres.XsumPhsAllCV{iprobe,g,kiter}, popres, true, [], phsSmthWin, phsSmthWin, true, true);%NaN, NaN, true, true);%
                                ...
                                [PostErrPhsAll2_iter,~,...
                                    PostErrpredPhsAll2_iter,~,~,~]...
                                    = ComputePostAve(popres.ErrXPhsAll2CV{iprobe,g,kiter}, popres.XsumPhsAllCV{iprobe,g,kiter}, popres, true, [], phsSmthWin, phsSmthWin, true, true);%NaN, NaN, true, true);%
                                PostErrpredPhsAll2_iter = PostErrpredPhsAll2_iter - mean(PostErrpredPhsAll2_iter);
                                PostErrpredNormPhsAll2_iter = PostErrpredPhsAll2_iter./std(PostErrpredPhsAll2_iter);
                                
                                PostErrPhsAll2_std = PostErrPhsAll2_std + (nfold - 1)/nfold*(PostErrPhsAll2_iter - popres.ErrXPhsAll2{iprobe,g}).^2;
                                PostErrpredPhsAll2_std = PostErrpredPhsAll2_std + (nfold - 1)/nfold*(Prange/(2*pi)*circ_dist(2*pi/Prange*PostErrpredPhsAll2_iter,2*pi/Prange*(popres.ErrXpredPhsAll2{iprobe,g}-mean(popres.ErrXpredPhsAll2{iprobe,g})))).^2;
                                PostErrpredNormPhsAll2_std = PostErrpredNormPhsAll2_std + (nfold - 1)/nfold*(Prange/(2*pi)*circ_dist(2*pi/Prange*PostErrpredNormPhsAll2_iter,2*pi/Prange*popres.ErrXpredNormPhsAll2{iprobe,g})).^2;
                                ...
                                [PostBiasPhsAll_iter,~,...
                                    ...
                                    = ComputePostAve(popres.BiasXPhsAllCV{iprobe,g,kiter}, popres.XsumPhsAllCV{iprobe,g,kiter}, popres, true, [], phsSmthWin, phsSmthWin, true, true);%NaN, NaN, true, true);%
                            ...
                            popres.ErrXPhsAll2_SE{iprobe,g} = sqrt(PostErrPhsAll2_std);
                            popres.ErrXpredPhsAll2_SE{iprobe,g} = sqrt(PostErrpredPhsAll2_std);
...
function [PostXAll_out, PostErrAll_out, PostBiasAll_out,...
    ...
    for k = 1:ndim4    
        ...
        if params.FsmoothOutput
            ...
            PostXAll = nanmean(PostXAll_all,3);
            PostErrAll = nanmean(PostErrAll_all,3);
            PostBiasAll = nanmean(PostBiasAll_all,3);
        end
...
%         PostCorrAlltemp([1:floor(size(PostCorrAll,1)/4) floor(size(PostCorrAll,1)*3/4):end],:) = 0;
        ...
        for kiter = 1:nfolds
            if params.FsmoothOutput
                ...
                PostXAll_iter = maskmat.*nanmean(PostXAll_all(:,:,~ismember(allsessions,kiter)),3);
                PostErrAll_iter = nanmean(PostErrAll_all(:,:,~ismember(allsessions,kiter)),3);
                PostBiasAll_iter = nanmean(PostBiasAll_all(:,:,~ismember(allsessions,kiter)),3);
            end
%             PostCorrAll_iter = special_smooth2D(nanmean(PostCorrAll_all(:,:,~ismember(allsessions,kiter)),3), [Ysmthbin/size(PostCorrAll_all,1) Xsmthbin/size(PostCorrAll_all,2)],[YFcircular XFcircular]);
            PostCorrAll_iter = nanmean(PostCorrAll_all(:,:,~ismember(allsessions,kiter)),3);
%             if ~isempty(popref)
