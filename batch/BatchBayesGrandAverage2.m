...
if nargin < 2
    prompt = {'Speed Threshold';'nphsbins';'nspdbins';'neyebins';'Window size (ms)';'Spatial smth (%)';'Decoding window size (ms)';'Decoding bins number';'suffix';'error type'};
    dlg_title = 'Parameters';
    num_lines = 1;
    def = {'5';'1';'5';'1';'15';'4';'50';'100';'goodonly';'DecPost'};
    ...
    if ~isempty(nruns)
        ...
        res.nDecbins = str2num(nruns{8});
        res.cellstr = nruns{9};
        res.DecAveType = nruns{10};%'DecPost' or 'DecError'
    end
else
    ...
    res.nDecbins = params.nDecbins;
    res.cellstr = params.cellstr;
    res.DecAveType = params.DecAveType;%'DecPost' or 'DecError'
end
savedfilename_res = ['D:\DATA\batch\All\Decoding\res_' res.DecAveType '_Twin' num2str(res.Tsmthwin) '_Xwin' num2str(res.Xsmthwin) '_spdth' num2str(res.SpeedThreshold)...
                     '_Decwin' num2str(res.Tsmthwin_dec) '_nDecbins' num2str(res.nDecbins) '_' num2str(res.nspeedbins) 'speedbins_' num2str(res.neyebins) 'eyebins_' num2str(res.nphsbins) 'thetabins_' res.cellstr '.mat'];
savedfilename_popres_sessions = ['D:\DATA\batch\All\Decoding\popres_sessions_' res.DecAveType '_Twin' num2str(res.Tsmthwin) '_Xwin' num2str(res.Xsmthwin) '_spdth' num2str(res.SpeedThreshold)...
                     '_Decwin' num2str(res.Tsmthwin_dec) '_nDecbins' num2str(res.nDecbins) '_' num2str(res.nspeedbins) 'speedbins_' num2str(res.neyebins) 'eyebins_' num2str(res.nphsbins) 'thetabins_' res.cellstr '.mat'];
savedfilename_popres_trials = ['D:\DATA\batch\All\Decoding\popres_trials_' res.DecAveType '_Twin' num2str(res.Tsmthwin) '_Xwin' num2str(res.Xsmthwin) '_spdth' num2str(res.SpeedThreshold)...
                     '_Decwin' num2str(res.Tsmthwin_dec) '_nDecbins' num2str(res.nDecbins) '_' num2str(res.nspeedbins) 'speedbins_' num2str(res.neyebins) 'eyebins_' num2str(res.nphsbins) 'thetabins_' res.cellstr '.mat'];
...
%     res.cellstr = 'goodonly_spktimes';
    ...
    filesuffix_EXP = ['Twin' num2str(res.Tsmthwin) '_' 'Xwin' num2str(res.Xsmthwin) '_' 'spdth' num2str(res.SpeedThreshold) '_' 'Decwin' num2str(res.Tsmthwin_dec) '_' 'nDecbins' num2str(res.nDecbins) '_' num2str(res.nspeedbins) 'speedbins' '_' num2str(res.neyebins) 'eyebins' '_' num2str(res.nphsbins) 'thetabins' '_' res.cellstr];
end
...
%             Slat = load([datadir filesep animalname filesep num2str(expt(ianimal).series{iseries}) filesep 'decoderParams_latency.mat']);
            ...
            res.LFPphase{ianimal,iseries} = EXP.data.es.LFPphase2(:,1);
...
%                 end
                ...
                for g = [2 1 3]
                    ...
                    res.tidx_licks{ianimal,iseries,iprobe,g} = false(nTimes,1);
                    for cont = 1:numel(cont_list)
                        for r = 1:numel(RL_list)
                            for o = 1:numel(outcome_list)
                                ...
                                tidx_licks = EXP.getSubsets(cont_list(cont),g,RL_list(r),outcome_list(o), -inf);
                                res.tidx_licks{ianimal,iseries,iprobe,g}(tidx_licks) = true;
                            end
                        end
                    end
                    ...
                    if sum(res.tidx{ianimal,iseries,iprobe,g}) > 0
                        ...
                        if g==2
                            nSec_blank = 6;
                            res.PostXSum_blankOnset{ianimal,iseries,iprobe} = zeros(Prange, EXP.Bayes.sampleRate*nSec_blank+1);
                            res.XSum_blankOnset{ianimal,iseries,iprobe} = zeros(1, EXP.Bayes.sampleRate*nSec_blank+1);
                            res.PostXSum_blankOffset{ianimal,iseries,iprobe} = zeros(Prange, EXP.Bayes.sampleRate*nSec_blank+1);
                            res.XSum_blankOffset{ianimal,iseries,iprobe} = zeros(1, EXP.Bayes.sampleRate*nSec_blank+1);
                            
                            blank_onset = find(diff(~EXP.Subset.noBlanks)>0 & EXP.data.es.outcome(1:end-1)==2);
                            ishift = 0;
                            for tshift = -(nSec_blank/2*EXP.Bayes.sampleRate):(nSec_blank/2*EXP.Bayes.sampleRate)
                                ishift = ishift+1;
                                for tt = 1:numel(blank_onset)
                                    if blank_onset(tt)+tshift>0 && blank_onset(tt)+tshift<=size(EXP.Bayes.Posterior0{iprobe,1},1)
                                        if (~EXP.Subset.noBlanks(blank_onset(tt)+tshift) || tshift<=0) && sum(isnan(EXP.Bayes.Posterior0{iprobe,1}(blank_onset(tt)+tshift,:)),2)==0
                                            res.PostXSum_blankOnset{ianimal,iseries,iprobe}(:,ishift) = res.PostXSum_blankOnset{ianimal,iseries,iprobe}(:,ishift)+EXP.Bayes.Posterior0{iprobe,1}(blank_onset(tt)+tshift,:)';
                                            res.XSum_blankOnset{ianimal,iseries,iprobe}(ishift) = res.XSum_blankOnset{ianimal,iseries,iprobe}(ishift)+1;
                                        end
                                    end
                                end
                            end
                            blank_offset = find(diff(~EXP.Subset.noBlanks)<0 & EXP.data.es.outcome(1:end-1)==2);
                            ishift = 0;
                            for tshift = -(nSec_blank/2*EXP.Bayes.sampleRate):(nSec_blank/2*EXP.Bayes.sampleRate)
                                ishift = ishift+1;
                                for tt = 1:numel(blank_offset)
                                    if blank_offset(tt)+tshift>0 && blank_offset(tt)+tshift<=size(EXP.Bayes.Posterior0{iprobe,1},1)
                                        if (~EXP.Subset.noBlanks(blank_offset(tt)+tshift) || tshift>=0) && sum(isnan(EXP.Bayes.Posterior0{iprobe,1}(blank_offset(tt)+tshift,:)),2)==0
                                            res.PostXSum_blankOffset{ianimal,iseries,iprobe}(:,ishift) = res.PostXSum_blankOffset{ianimal,iseries,iprobe}(:,ishift)+EXP.Bayes.Posterior0{iprobe,1}(blank_offset(tt)+tshift,:)';
                                            res.XSum_blankOffset{ianimal,iseries,iprobe}(ishift) = res.XSum_blankOffset{ianimal,iseries,iprobe}(ishift)+1;
                                        end
                                    end
                                end
                            end
                        ...
                        res.maxPost{ianimal,iseries,iprobe,g} = NaN(1, Xrange);
                        for i = 1:Xrange
                            res.maxdecErr{ianimal,iseries,iprobe,g}(i) = Prange/(2*pi)*(circ_std(circ_dist(2*pi/Prange*EXP.Bayes.MaxDecodedPosition0{iprobe,1}(Xtidx(:,i) & res.tidx{ianimal,iseries,iprobe,g} & goodPostidx),2*pi/Prange*res.X{ianimal,iseries}(Xtidx(:,i) & res.tidx{ianimal,iseries,iprobe,g} & goodPostidx))));
                            res.meandecErr{ianimal,iseries,iprobe,g}(i) = Prange/(2*pi)*(circ_std(circ_dist(2*pi/Prange*EXP.Bayes.MeanDecodedPosition0{iprobe,1}(Xtidx(:,i) & res.tidx{ianimal,iseries,iprobe,g} & goodPostidx),2*pi/Prange*res.X{ianimal,iseries}(Xtidx(:,i) & res.tidx{ianimal,iseries,iprobe,g} & goodPostidx))));
                            res.maxPost{ianimal,iseries,iprobe,g}(i) = nanmean(max(EXP.Bayes.Posterior0{iprobe,1}(Xtidx(:,i) & res.tidx{ianimal,iseries,iprobe,g} & goodPostidx,:),[],2));
                        end
...
%                         end
                        ...
                        if res.Tsmthwin_dec <= 50
                            ...
                            POdec2 = zeros(res.nPhsbins,Prange,numel(minidx));
                            ...
                            ThetaPosError0 = EXP.Bayes.PosError0{iprobe};
                            for i = 1:Xrange
                                xidx = EXP.Bayes.X0{1} == i;
                                ThetaPosError0(xidx,:) = ThetaPosError0(xidx,:) - repmat(nanmean(ThetaPosError0(xidx,:),1),[sum(xidx) 1]);
                            end
                            
                            for tt = 1:numel(minidx)
                                if minidx(tt) > 1 && minidx(tt)+nbtimebins-1 < size(ThetaPosError0,1)
                                    if strcmp(res.DecAveType,'DecPost')
                                        ...
                                        POmap2 = ThetaPosError0(max(1,minidx(tt)):min(minidx(tt)+nbtimebins-1,end),:);
                                        POdec2(:,:,tt) = interp1((0:nbtimebins-1)/thetaperiod(tt),POmap2,0:1/res.nPhsbins:(1-(1/res.nPhsbins)));
                            ...
                            res.ErrXSumPhsAll2{ianimal,iseries,iprobe,g} = zeros(Prange, res.nPhsbins, XrangePhs);
                            ...
                            for i = 1:XrangePhs
                                for iphs = 1:res.nPhsbins
                                    ...
                                    if strcmp(res.DecAveType,'DecPost')
                                        ...
                                        res.ErrXSumPhsAll2{ianimal,iseries,iprobe,g}(:,iphs,i)= nansum(POdec2(iphs,:,ttidx),3)';
                            ...
                            for kiter = 1:CVO.kfold
                                ...
                                res.ErrXSumPhsAll2CVO{ianimal,iseries,iprobe,g,kiter} = zeros(Prange, res.nPhsbins, XrangePhs);
                                ...
                                for i = 1:XrangePhs
                                    for iphs = 1:res.nPhsbins
                                        ...
                                        if strcmp(res.DecAveType,'DecPost')
                                            ...
                                            res.ErrXSumPhsAll2CVO{ianimal,iseries,iprobe,g,kiter}(:,iphs,i) = nansum(POdec2(iphs,:,ttidx),3)';
