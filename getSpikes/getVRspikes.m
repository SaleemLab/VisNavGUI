...
if run_rest
    ...
    load([dDIRname filesep fname '_screenTimes2']);%load([dDIRname filesep fname '_screenTimes']);
    ...
    if ~isempty(es_test)
        RecToVR_correction = screenTimes(1) - es_test.screenTimes2(1);%screenTimes(1) - es_test.screenTimes2(2);
        screenTimes = screenTimes - RecToVR_correction;
        
        screenTimeStamp = zeros(1,round(max(screenTimes*1000))+1);
        screenTimeStamp(round(screenTimes*1000)+1) = 1;
        screenTimeStamp2 = zeros(1,round(max(es_test.screenTimes2*1000))+1);
        screenTimeStamp2(round(es_test.screenTimes2*1000)+1) = 1;
        corrScreenTimes = xcorr(smooth(screenTimeStamp,100),smooth(screenTimeStamp2,100),10000);
        [~,idx] = max(corrScreenTimes);
        disp(['remaining lag : ' num2str((idx - 10001)) ' ms']);
    else
    ...
    if exist([dDIRname filesep fname '_LFP6.mat']) && FloadVRfile
        %_LFP6.mat is filtered between 6Hz and 9Hz, with correct
        %synchronization
        %_LFP7.mat is filtered between 6Hz and 12Hz, with correct
        %synchronization but no smoothing before downsampling
        %_LFP8.mat is filtered between 6Hz and 9Hz, with correct
        %synchronization but no smoothing before downsampling
        %_LFP9.mat is filtered between 5Hz and 10Hz, with correct
        %synchronization
        try
        nChCA1 = 34;%33;
        LFPfile = matfile([dDIRname filesep fname '_LFP6.mat']);
        es.LFPphase_Ch = mod(double(LFPfile.LFPthetaPhase)*2,360)';
    ...
    if exist([dDIRname filesep fname '_V1_LFP6.mat'])%([dDIRname2 filesep fname '_V1_LFP.mat'])
%         try
...
%         end
    ...
    es.spiketrajPercent = NaN(length(es.sampleTimes), numCells);
...
%         display(['Processing cell: ' num2str(ichan)]);
        ...
        st = round(chans(ichan).spiketimes./(1/samplerate));%ceil(chans(ichan).spiketimes./(1/samplerate));%
...
%             end
        ...
        spiketrajPercent = mod(interp1(es.sampleTimes,unwrap(es.trajPercent/round(max(es.trajPercent))*2*pi)*round(max(es.trajPercent))/(2*pi),chans(ichan).spiketimes,'linear'),round(max(es.trajPercent)));
        es.spiketrajPercent(ust,ichan) = spiketrajPercent(min(end,idx));
%         if isfield(chans(ichan),'spikephaseV1')
...
%         end
        ...
        for irep = 1:length(repeats)
            ...
            if isfield(chans(ichan),'spikephase')
                es.spikePhase(repeats(irep),ichan) = mod(360/(2*pi)*circ_mean(2*pi/360*[es.spikePhase(repeats(irep),ichan);chans(ichan).spikephase(min(repspknum(irep)+1,end))]),360);
            end
            es.spiketrajPercent(repeats(irep),ichan) = mod(maxtraj/(2*pi)*circ_mean(2*pi/maxtraj*[es.spiketrajPercent(repeats(irep),ichan);spiketrajPercent(min(repspknum(irep)+1,end))]),maxtraj);
%             if isfield(chans(ichan),'spikephaseV1')
...
%             end
        ...
        es.spiketrajPercent(isnan(es.spiketrajPercent(:,ichan)),ichan) = es.trajPercent(isnan(es.spiketrajPercent(:,ichan)));
...
%     end
    if length(es.screenTimes) ~= length(es.screenTimes2)
        disp(['!!!!!WARNING!!!!!!: ' num2str(animal) '_' num2str(iseries) '_' num2str(iexp) ' trajectory has ' num2str(length(es.screenTimes) - length(es.screenTimes2)) ' more frames than screen refreshes']);
    end
