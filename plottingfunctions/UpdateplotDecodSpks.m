function UpdateplotDecodSpks(PlotVar,EXP,Layout)
PlotVar.Plots = [];
es = EXP.data.es;
...
%                         end
%                         tidx = blanks & EXP.data.es.smthBallSpd > EXP.Bayes.speed_th & ~isnan(EXP.data.es.smthBallSpd) & ~isnan(EXP.data.es.smthTrajSpd) & EXP.data.es.outcome == 2;
%                         tidx = tidx & EXP.Bayes.X == mode(EXP.Bayes.X(tidx));
%                         
%                         blankdist = NaN(size(EXP.data.es.smthBallSpd));
%                         idxblankstart = find(diff(EXP.Subset.noBlanks>0));
%                         for iblank = 1:numel(idxblankstart)
% %                             if idxblankstart(iblank)-49>=1 && idxblankstart(iblank)+50 <= numel(blankdist)
% %                                 blankdist(idxblankstart(iblank)-49:idxblankstart(iblank)+50) = 1:100;
% %                             end
%                             if iblank+1<=numel(idxblankstart)
%                                 blankdist(idxblankstart(iblank):idxblankstart(iblank+1)-1) = EXP.data.es.trajPercent(idxblankstart(iblank):idxblankstart(iblank+1)-1) + cumsum(EXP.data.es.smthBallSpd(idxblankstart(iblank):idxblankstart(iblank+1)-1))/EXP.data.es.sampleRate*0.5;
%                             else
%                                 blankdist(idxblankstart(iblank):end) = EXP.data.es.trajPercent(idxblankstart(iblank):end) + cumsum(EXP.data.es.smthBallSpd(idxblankstart(iblank):end))/EXP.data.es.sampleRate*0.5;
%                             end
%                         end
                        ...
                        if strcmp(PlotVar.ChosenObj{k}, 'Posterior x X') || strcmp(PlotVar.ChosenObj{k}, 'Posterior x D')
                            ...
                            elseif strcmp(PlotVar.ChosenObj{k}, 'Posterior x D')
                                X = floor(blankdist)+1;%blankdist;%EXP.Bayes.D;
                                Xrange = 100;%max(floor(X));
                                gainratio = unique(es.gain(tidx)/mode(es.gain));
...
%                             goodidx = goodidx & sum(EXP.data.es.spikeTrain(:,EXP.Bayes.DecCellidx{iprobe})>0,2)>5;
...
%                             for i = 1:Xrange
%                                 Pprior(i) = sum(X == i  & tidxref & goodidx);
%                             end
%                             Pprior = Pprior/sum(tidxref & goodidx);
%                             Post = Post.*repmat(Pprior,[size(Post,1) 1]);
%                             Post = Post./repmat(nansum(Post(:,:),2),[1 Xrange]);
%                             Post = Post/(1/Xrange);

%                             Pprior = zeros(size(Post));
%                             for i = 1:Xrange
%                                 Pprior(:,i) = smooth(Post(:,i),60*20+1);
%                             end
%                             Post = Post.*Pprior;

%                             Post = Post.*circshift(Post,+1,1);
%                             Post = Post./repmat(nansum(Post,2),[1 size(Post,2)])/(1/Prange);

%                             Post = Post./repmat(nanmean(Post(tidxref & goodidx,:),1),[size(Post,1) 1]);
                            ...
                            if ~PlotVar.FdecXdistribution
                                for i = 1:Xrange
                                    mat(:,i) = nanmean(Post(Xtidx(:,i) & tidx & spdidx & phsidx & goodidx,:),1)';%nanmean(Post(X == i  & tidx & spdidx & phsidx & goodidx,:),1)';
                                end
                                
%                                 disp(['max decoding error (electrode#' num2str(selprobe) '): ' num2str(nanmean(Prange/(2*pi)*abs(circ_dist(maxdecErr,circ_mean(maxdecErr))))) ' cm'])
%                                 disp(['mean decoding error(electrode#' num2str(selprobe) '): ' num2str(nanmean(Prange/(2*pi)*abs(circ_dist(meandecErr,circ_mean(meandecErr))))) ' cm'])

%                                 disp(['max posterior (electrode#' num2str(selprobe) '): ' num2str(nanmean(maxPost)) ' Prob./chance'])
...
%                                 Xpred = EXP.Bayes.MaxDecodedPosition0{selprobe,1};%(EXP.Bayes.DecodingError0{selprobe,1}+floor(Prange/2));%EXP.Bayes.DecodingBias0{iprobe,1}+floor(Prange/2);%
                                ...
                                F = F./repmat(XSum*(1/Prange),[Prange 1]);
                                F = F./repmat(sum(F,1),[size(F,1) 1])/(1/Prange);
%                                 F = F./repmat(XSum*(1/Prange),[Prange 1]);%special_smooth2D(F, [lambdaSmooth/Prange lambdaSmooth/Xrange],[true true])./special_smooth2D(repmat(XSum*(1/100),[Prange 1]), [lambdaSmooth/Prange lambdaSmooth/Xrange],[true true]);%
                        ...
                        if strcmp(PlotVar.ChosenObj{k}, 'Theta x Error x X') || strcmp(PlotVar.ChosenObj{k}, 'Theta x Post x X')
                            ...
                            elseif strcmp(PlotVar.ChosenObj{k}, 'Theta x Error x X') || strcmp(PlotVar.ChosenObj{k}, 'Theta time x Error x X')
                                ...
                                if strcmp(PlotVar.ChosenObj{k}, 'Theta x Error x X')
                                    Fusephase = true;%false;%
                                else
                                    Fusephase = false;
                                end
                            end
%                             Post = EXP.Bayes.Posterior0{selprobe,Posttype};
%                             Post = Post./repmat(nanmean(Post(tidx & spdidx,:),1),[size(Post,1) 1]);
%                             for tt = 1:size(Post,1)
%                                 Post(tt,:) = circshift(Post(tt,:),50 - EXP.Bayes.Xsmth0{Xtype}(tt));
%                             end
                            X = EXP.Bayes.Xsmth0{Xtype};
                            Prange = size(Post,2);
                            Xrange = max(X);
