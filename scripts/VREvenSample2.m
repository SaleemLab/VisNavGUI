function es = VREvenSample2(VRdata, sampleTimes, screenTimes,samplerate)
...
es.sortIDX = es.sortIDX(es.screenTimes2>0);
es.screenTimes2 = es.screenTimes2(es.screenTimes2>0);
if nargin < 4
    samplerate = 60;
end
% es.sampleRate = '60 Hz';
...
%     end    
    ...
    minNscreenTimes = min(numel(es.screenTimes),numel(es.screenTimes2));
    es.screenTimes = es.screenTimes(1:minNscreenTimes)';
    es.sortIDX = es.sortIDX(1:minNscreenTimes);
    es.screenTimes2 = es.screenTimes2(1:minNscreenTimes);
    delay = NaN(1,11);
    icount = 0;
    for ishift = -5:5
        icount = icount + 1;
        if ishift < 0
            delay(icount) = nanmean(es.screenTimes2(1:(end + ishift))-es.screenTimes(1-ishift:end));
        else
            delay(icount) = nanmean(es.screenTimes2((1+ishift):end)-es.screenTimes(1:(end-ishift)));
        end
    end
    [delaymin,bestdelay] = min(abs(delay));
    disp(['best delay of ' num2str(delaymin*1000) 'ms achived with shift of ' num2str(bestdelay-6)])
end

% [vmax,imax] = max(xcorr(es.screenTimes2(1:minNscreenTimes),es.screenTimes(1:minNscreenTimes),30,'coeff'));
% if vmax < 0.999 || abs(imax - 31) > 2
%     warning('additional screen times may be not at the end');
% end

evenSampleTime = min(es.screenTimes):(1/es.sampleRate):max(es.screenTimes);
