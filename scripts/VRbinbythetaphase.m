...
%         end
    ...
    %     idxphschange = [(1:2:numel(LFPbinphase))' ; numel(LFPbinphase)+1];
        ...
        maxtrajPercentunwrapped = round(max(es_in.trajPercentunwrapped));
        ...
        es_out.trajPercentunwrapped = zeros(nbins,1);
        ...
        es_out.completetrial = zeros(nbins,1);
        ...
        es_out.spikePhase = zeros(nbins,size(es_in.spikePhase,2));
        ...
        for i = 1:numel(idxphschange)-1
            ...
            es_out.trajPercentunwrapped(i)  = mod(nanmean(unwrap(es_in.trajPercentunwrapped(idxmerge)/maxtrajPercentunwrapped*2*pi)*maxtrajPercentunwrapped/(2*pi)),maxtrajPercentunwrapped);
            ...
            es_out.completetrial(i) = max(es_in.completetrial(idxmerge));
            ...
            es_out.spikePhase(i,:) = max(es_in.spikePhase(idxmerge,:),[],1);
            es_out.mua(i) = nansum(es_in.mua(idxmerge));

            es_out.LFPbinphase(i) = 180 + 360/(2*pi)*circ_mean(2*pi/360*LFPbinphase(idxmerge));
            es_out.LFPphase(i,:) = es_out.LFPbinphase(i);
        ...
        es_out.bestchan = es_in.bestchan;
        es_out.waveform = es_in.waveform;
