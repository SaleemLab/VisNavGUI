function [es, VRDetails, chans, TempChans] = ...
...
dDIRname = [DIRS.data filesep animal filesep num2str(iseries)];
run_rest = 0;
...
if run_rest == 1
    ...
    [VRdata, VRdata_o, es] = VRWheelLoad_SL(animal, iseries, 101);
    iexp_list = dir([DIRS.ball filesep animal filesep num2str(iseries) filesep animal '_' num2str(iseries) '*.mat']);
    if length(iexp_list)>1
        for gg = 2:length(iexp_list)
            [VRdata, VRdata_o, es_x] = VRWheelLoad_SL(animal, iseries, str2double(iexp_list(gg).name(end-15:end-13)));
            if gg == 2
    ...
    [VRdata, VRdata_o, ~] = VRWheelLoad_SL(animal, iseries, VR_SessionOI);
    ...
    for igroup_idx = 1:length(igroup)
        if exist('iaddinfo')
            temp = getKwikSpikes(animal, iseries, iexp, igroup(igroup_idx)-1,iaddinfo);
        else
            temp = getKwikSpikes(animal, iseries, iexp, igroup(igroup_idx)-1);
        end
    ...
    FileNameMetaDat = dir([DIRS.multichanspikes filesep animal filesep 'DAT' filesep num2str(iseries) filesep '*.dat_meta.mat']);
    ...
    for nc = 1:length(chans)
        if strcmp(chans(nc).id(5),'Good')|| strcmp(chans(nc).id(5),'MUA')% Make this 'not' Noise
