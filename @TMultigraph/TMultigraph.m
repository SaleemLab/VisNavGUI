...
properties
    %handle for main figure
    parentfig
    %handle for file menu in figure toolbar
    FileMenu
    %handle for analysis menu in figure toolbar
    AnalysisMenu
    %handle for plot menu in figure toolbar
    PlotMenu
    %
    parentgrid
    %uix.TabPanel object
 ...
 methods
     function obj = TMultigraph(name, title, figurehandle)
         ...
         if nargin < 3
             figurehandle = figure('Units','normalized','Position',[0 0 1 1],'Name',name,'NumberTitle','off','MenuBar','none','ToolBar','none');
         end
         if nargin < 2
             title = 'Page 1';
         end
         ...
         obj.parentgrid = uix.GridFlex('Parent',obj.parentfig, 'Spacing', 2, 'Padding', 2, 'BackgroundColor', [1 1 1] );
         obj.tabpanel = uix.TabPanel('Parent', obj.parentgrid, 'Padding', 2, 'TabWidth', 100, 'BackgroundColor', [0.8 0.8 0.8] );
         pagecount = 1;
         obj.page = uix.CardPanel('Parent',obj.tabpanel);
         obj.pageLayout{pagecount} = uix.HBoxFlex('Parent',obj.page(pagecount), 'Spacing', 2, 'Padding', 2 , 'BackgroundColor', [1 1 1] );
         obj.window{1,1} = uix.GridFlex('Parent',obj.pageLayout{pagecount}, 'Spacing', 2, 'Padding', 2, 'BackgroundColor', [1 1 1] );
         obj.tabpanel.TabTitles(1) = {title}; 
         
         obj.FileMenu = uimenu('Parent',figurehandle,'Label','File');
         obj.AnalysisMenu = uimenu('Parent',figurehandle,'Label','Analysis');
         obj.PlotMenu = uimenu('Parent',figurehandle,'Label','Plots');
     end
     
     function obj = reset(obj)
         delete(obj.parentgrid);
         delete(obj.tabpanel);
         obj.parentgrid = uix.GridFlex('Parent',obj.parentfig, 'Spacing', 2, 'Padding', 2, 'BackgroundColor', [1 1 1] );
         obj.tabpanel = uix.TabPanel('Parent', obj.parentgrid, 'Padding', 2, 'TabWidth', 100, 'BackgroundColor', [0.8 0.8 0.8] );
         pagecount = 1;
     ...
     function obj = updateTitle(obj, name)
         name = ['VisNav - ' name];
         set(obj.parentfig,'Name',name);
     end
     
     function obj = addPage(obj, title)
         pagecount = numel(obj.page) + 1;
         if nargin < 2
             title = ['Page ' num2str(pagecount)];
         end
         obj.page(pagecount) = uix.CardPanel('Parent',obj.tabpanel);
         obj.pageLayout{pagecount} = uix.HBoxFlex('Parent',obj.page(pagecount), 'Spacing', 2, 'Padding', 2, 'BackgroundColor', [1 1 1] );
         obj.window{pagecount,1} = uix.GridFlex('Parent',obj.pageLayout{pagecount}, 'Spacing', 2, 'Padding', 2, 'BackgroundColor', [1 1 1] );
     ...
     function obj = addPermWindow(obj,str,dim)
         ...
         switch str
             case {'Horizontal','H'}
                 obj.permwindow{permwincount} = uix.HBoxFlex('Parent',obj.parentgrid, 'Spacing', 2, 'Padding', 2, 'BackgroundColor', [1 1 1] );
                 obj.parentgrid.Contents = [obj.parentgrid.Contents(end);obj.parentgrid.Contents(1:end-1)];
                 set(obj.parentgrid, 'Heights', -dim, 'Spacing', 2 );
             case {'Vertical','V'}
                 obj.permwindow{permwincount} = uix.VBoxFlex('Parent',obj.parentgrid, 'Spacing', 2, 'Padding', 2, 'BackgroundColor', [1 1 1] );
                 obj.parentgrid.Contents = [obj.parentgrid.Contents(end);obj.parentgrid.Contents(1:end-1)];
                 set(obj.parentgrid, 'Widths', -dim, 'Spacing', 2 );
         end
     end
     ...
     function obj = Hdividepage(obj, pagenum, nwin, dim)
         ...
         obj.pageLayout{pagenum} = uix.HBoxFlex('Parent',obj.page(pagenum), 'Spacing', 2, 'Padding', 2, 'BackgroundColor', [1 1 1]);
         for w = 1:nwin
             obj.window{pagenum,w} = uix.GridFlex('Parent',obj.pageLayout{pagenum}, 'Spacing', 2, 'Padding', 2, 'BackgroundColor', [1 1 1]);
         end
         set( obj.pageLayout{pagenum}, 'Widths', -dim, 'Spacing', 2 );
     end 
     
     function obj = Vdividepage(obj, pagenum, nwin, dim)
         delete(obj.page(pagenum).Children);
         obj.pageLayout{pagenum} = uix.VBoxFlex('Parent',obj.page(pagenum), 'Spacing', 2, 'Padding', 2, 'BackgroundColor', [1 1 1]);
         for w = 1:nwin
             obj.window{pagenum,w} = uix.GridFlex('Parent',obj.pageLayout{pagenum}, 'Spacing', 2, 'Padding', 2, 'BackgroundColor', [1 1 1]);
         end
         set( obj.pageLayout{pagenum}, 'Heights', -dim, 'Spacing', 2 , 'BackgroundColor', [1 1 1]);
     end 
     ...
     function obj = fig2pdf(obj,filename,Fappend,figname)
         ...
         if ~isempty(figname)
             fig2save = figure('Visible','on','Position',[Scrsize(1) Scrsize(2) Scrsize(3) Scrsize(4)],'name',figname,'renderer','opengl');
         else
             fig2save = figure('Visible','on','Position',[Scrsize(1) Scrsize(2) Scrsize(3) Scrsize(4)],'renderer','opengl');
         end
         ...
         print(fig2save,filename,'-dpdf', '-painters', '-r0')
